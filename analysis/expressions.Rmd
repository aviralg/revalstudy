...
```{r}
n_arithm_call_sites <- arithm_folded %>% pull(src_ref) %>% n_distinct()
...
```{r}
with_var <- E %>% filter(str_detect(expr_resolved, "^(?:\\.[:alpha]|[:alpha:])[[:alpha:][:digit:]_\\.]*$"))
```


We can categorize them by number of call sites and number of packages:
```{r}
n_with_var <- with_var %>% pull(src_ref) %>% n_distinct()

with_var %>% group_by(expr_resolved) %>% summarise(n_call_sites = n_distinct(src_ref), n_packages = n_distinct(ev_package)) %>% datatable()
```
