...
resolved_variety <- E %>% group_by(ev_expr, src_ref) %>% summarise(n = n_distinct(expr_resolved)) %>% arrange(desc(n)) 
resolved_variety %>% datatable()

nb_one_resolved <- resolved_variety %>% filter(n == 1) %>% nrow()
...
```{r}
one_resolved_type <- E %>% group_by(src_ref) %>% mutate(n_resolved = n_distinct(expr_resolved)) %>% filter(n_resolved == 1) %>% ungroup() %>% select(src_ref, n_resolved, type) %>% distinct()
...
```{r}
widespread_expr <- E %>% group_by(expr_resolved) %>% summarize(n_call_sites = n_distinct(src_ref), n_packages = n_distinct(ev_package))
```
