...
params:
  ...
  normalized_path: normalized-expressions.fst
...
read_fst(normalized_path) %>% as_tibble() -> N
...
# Normalization of resolved expressions


We join the normalized expressions and their statistics with the calls.

```{r}
E_cano <- E %>% left_join(N, by = c("hash" = "expr_resolved_hash")) # Common column is the hash of the full resolved expression
```
...
by_src_ref <- E_cano %>% group_by(src_ref) %>%
  summarise(n_resolved = n_distinct(hash), n_canonic = n_distinct(expr_canonic))
```

There should always be less normalized epxressions than resolved ones. For how many do we have strictly less normalized expressions?

```{r}
by_src_ref %>% filter(n_resolved > n_canonic) %>% arrange(desc(n_canonic))
```


```{r}
by_src_ref %>% filter(n_resolved == 1 | n_resolved > n_canonic)  %>% arrange(desc(n_canonic)) %>% print(n = 100)
```

```{r}
exprs_by_srcref <- E_cano %>% group_by(src_ref) %>% summarize(n_canonic = n_distinct(expr_canonic), exprs = paste0(unique(expr_canonic), collapse="    ;     "))
```
