...
canonic_expr <- function(exp) {
  if (is.call(exp)) {
    ...
    else if(length(function_name) == 1 && as.character(function_name) %in% comp_op && every(res, function(v) {v == "NUM"})) {
      return("BOOL")
    }
    else if(length(function_name) == 1 && as.character(function_name) %in% c("c", "list") && length(res) > 1 && n_distinct(res) == 1 ) { # && res[[1]] %in% c("BOOL", "NUM", "STR")
