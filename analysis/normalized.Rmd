...
widespread_minimized <- E_cano %>%
  group_by(minimized) %>%
  summarize(n_call_sites = n_distinct(src_ref), proportion = ratio(n_call_sites, nb_call_sites), n_packages = n_distinct(ev_package), n_operations = round(mean(n_op, na.rm=TRUE), 2), percent_envir = round(100 * sum(!is.na(envir)) / n(), 2))
