...
widespread_minimized <- E_cano %>%
  group_by(minimized) %>%
  summarize(n_call_sites = n_distinct(src_ref), n_packages = n_distinct(ev_package), n_operations = mean(n_op, na.rm=TRUE), prop_envir = sum(!is.na(envir)) / n())
...
There are `r nb_a_env_call_sites` call sites where `envir` is non-trivial, with an assignment.
...
There are `r nb_plotting_call_sites` plotting call sites.
...
This already amounts to `r nb_parse_plot_call_sites` call sites.
...
It happens in `r nb_infix_call_sites` call sites.
