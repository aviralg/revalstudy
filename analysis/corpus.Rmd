...
cran_runnable_code <- sum(ALL_CRAN$runnable_code)
cran_eval_calls <- sum(CRAN_eval_traces$nb_ev_calls)

overview_table(
  r("CRAN available packages", 15962), #length(available.packages()[,1])), # from the CRAN mirror
  ...
  r("CRAN undefined", UNDEFINED),
  r("CRAN undefined ratio", ratio(nrow(UNDEFINED), cran_eval_calls))
)
```
...
```{r}
overview_table(
  r("Corpus code", kaggle_code + cran_runnable_code)
)
```
