...
#  filter(evals > 0, evals_hit > 0) %>%
  ...
  ggplot(aes(x=evals, y=evals_hit, size=package_r_code, label=label)) + 
    ...
    geom_text(nudge_y = .15, check_overlap = TRUE, size=4) +
    scale_x_continuous(
      trans="log10",
      breaks=function(x) {
        f <- scales::breaks_log()
...
#    geom_smooth(method = "lm", se = FALSE, show.legend = FALSE, linetype="dashed", color="red", size=0.2) +
    labs(x="Eval call sites (log)", y="Traced eval call sites (log)", size="Package code size") +
