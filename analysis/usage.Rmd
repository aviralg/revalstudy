...
if (file.exists("Data/.lock.fst")) {
  saved <- read.fst("Data/.lock.fst")[[1,1]]
  if (saved == file.size(calls_path)) {
    read.fst("Data/E.fst") %>% as_tibble() -> E
    rebuild <- FALSE
  }
}
if (params$force_rebuild | rebuild) {
  ...
  write.fst(E, "Data/E.fst")
  sz <- file.size(calls_path) %>% as_tibble()
  write.fst(sz,"Data/.lock.fst")
}
...
```{r coverage-per-package}
cov_packages <- static_call_sites_per_package %>%
  left_join(sites_per_package, by = c("package" = "ev_package"), suffix = c("_static", "_tracer")) %>% 
  mutate(n_tracer = replace_na(n_tracer, 0L)) 

# if n_static < n_tracer, adjust n_tracer to n_static
# Probably due to higher-order functions taking some eval
cov_packages <- cov_packages %>%
  mutate(n_static_c = if_else(n_static < n_tracer, n_tracer, n_static))

cov_packages <- cov_packages %>% mutate(coverage = 100 * n_tracer / n_static_c)

cov_packages %>% ggplot() +
  geom_histogram(aes(x = coverage), binwidth=1) + 
  labs(x = "coverage (%)")

ggsave(path(PLOT_DIR, "coverage_per_package.pdf"))
```

What is the actual coverage vs the expected coverage?
```{r}
corpus_cov<- C_raw %>% select(package, size, revdeps)

corpus_cov<- corpus_cov %>% left_join(cov_packages, by = "package") %>%
  mutate(revdeps = replace_na(revdeps, 0))

corpus_cov %>% ggplot() +
  geom_point(aes(x = coverage, y = revdeps, size = size)) + 
  scale_y_log10() +
  scale_size_area() +
  labs(x = "coverage (%)", y = "Reverse dependencies")

ggsave(path(PLOT_DIR, "coverage_per_package_size.pdf"))
```

```{r}
corpus_cov %>% ggplot() +
  geom_point(aes(x = coverage, y = revdeps, size = n_static)) + 
  scale_y_log10() +
  scale_size_area() +
  labs(x = "coverage (%)", y = "Reverse dependencies", size = "Static call sites")

ggsave(path(PLOT_DIR, "coverage_per_package_static_call_sites.pdf"))
```
...
ggsave(path(PLOT_DIR, "size_loaded_distribution.pdf"))
```
...
ggsave(path(PLOT_DIR, "events_per_pack_large.pdf"))
```
...
ggsave(path(PLOT_DIR, "events_per_pack_small.pdf"))
```
...
languages <- types %>% filter(type == "language") %>% .$n
expressions <- types %>% filter(type == "expression") %>% .$n

ggplot2_symbs <- E %>% filter(ev_package == "ggplot2") %>% 
  count(type, wt=duplicates) %>% filter(type == "symbol") %>% .$n
