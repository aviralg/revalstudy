...
params:
  base_dir: Data/
  calls_path: base/summarized.fst
  corpus_path: corpus.fst
  static_path: package-evals-static.csv
  force_rebuild: TRUE
---
...
calls_path <- paste0(base_dir, params$calls_path)
corpus_path <- paste0(base_dir, params$corpus_path)
static_path <- paste0(base_dir, params$static_path)
```
...
rebuild <- TRUE
# We only read the orignal evals_calls file if we don't have a summarized
# version stored on disk.  NOTE: the heuristic to detect changes is file
# size. If that fails, manually delete `.lock.fst`
...
if (params$force_rebuild | rebuild) {
  ...
  if (!"direct_interpreter_eval" %in% names(E_raw)) { # We are dealing with
    E_raw %>% mutate(direct_interpreter_eval=0) -> E_raw # partial data.
  } #                                                  We create a fake column.
...
```{r eval-calls-violin-plot, include=F}
sites_per_package %>%
...
E %>%
  ...
  theme(text = element_text(size=17),
        axis.text.x = element_text(color = lbl_colour)) -> G
...
E %>% 
  ...
  theme(text = element_text(size=17),  panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(), aspect.ratio = 4/3) -> GG
...
tot_events <- E %>%
  summarise(n = sum(opcodes * duplicates)) %>%
  .$n
max_events <- E %>%
  summarize(n = max(opcodes)) %>%
  .$n
...
E %>%
  ...
  labs(y = "", x = "") +
  theme(legend.position = "none", text = element_text(size=25)) -> G
...
triggered %>%
  ...
  theme(text = element_text(size=16), axis.text = element_text(size=20), aspect.ratio = 4/3) -> G
