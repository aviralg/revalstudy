...
E_raw %>% select(
  ...
  arg_size = expr_resolved_length, #   Size in bytes of eval argument string
...
overview_table(
  ...
  r("Szninefive", nb_nodes_many$arg_size)
  ) -> TBL

sizes_prop %>%
  ggplot(aes(x=arg_size)) + geom_histogram() 
...
tot_events <- E %>% summarise(n=sum(opcodes)) %>% .$n
max_events <- E %>% summarize(n = max(opcodes)) %>% .$n

less_50_events <- E %>% filter(opcodes <= 50) %>%
  count(wt = duplicates) %>%
  .$n
...
triggered <- E %>%
  group_by(src_ref) %>%
  summarize(n_runs = n_distinct(file), 
            n = sum(duplicates), 
            average = n / n_runs) %>%
