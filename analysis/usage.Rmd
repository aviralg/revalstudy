...
triggered %>%
  ...
  theme(text = element_text(size=16), axis.text = element_text(size=20), aspect.ratio = 4/3)
...
ggplot2_symbs <- E %>% filter(ev_package == "ggplot2") %>% count(type, wt=duplicates) %>% filter(type == "symbol") %>% .$n
