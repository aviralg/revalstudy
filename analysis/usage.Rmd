...
The size of our corpus is `corpus_size` packages.
...
(note that we may not trigger all `eval`s in a package due to spotty covereage).
...
hardly use eval while other use it intensily. The numbers are "for all runs",
...
r("binf", ev_summary[[6]]),
r("bing", ev_summary[[7]]),
r("binh", ev_summary[[8]])) -> TBL
```

The distribution of eval calls, each bin represents a power of 10, the number in the bin is the count of packages.
