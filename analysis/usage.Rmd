...
more_1000_calls <- eval_calls_table %>% filter(n > 1000) %>% nrow()

overview_table(
  r("eval calls less hundred", ev_summary[[1]] + ev_summary[[2]]),
  r("eval calls more thousand", more_1000_calls)
)
```
...
TBL <- overview_table(
  ...
  r("at least one call site", sites_per_package$n),
