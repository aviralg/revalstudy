...
params:
  ...
  calls_packages_path: /run/package-evals-traced.10/summarized-packages.fst
  corpus_path: /run/package-evals-traced.10/corpus.fst
---
...
calls_packages_path <- paste0(params$base_dir, params$calls_packages_path)
...
nb_eval_pres_calls <- own_eval_calls %>%
  filter(eval_function == "eval") %>%
  count(wt = nb_ev_calls)
nb_evalq_calls <- own_eval_calls %>%
  filter(eval_function == "evalq") %>%
  count(wt = nb_ev_calls)
nb_eval.parent_calls <- own_eval_calls %>%
  filter(eval_function == "eval.parent") %>%
  count(wt = nb_ev_calls)
nb_local_calls <- own_eval_calls %>%
  filter(eval_function == "local") %>%
  count(wt = nb_ev_calls)

eval_at_least_once <- own_eval_calls %>%
  count(package, wt = nb_ev_calls) %>%
  nrow()
...
nb_ev_call_sites <- eval_calls %>%
  select(eval_call_srcref) %>%
  n_distinct(na.rm = TRUE)

call_sites_per_package <- eval_calls %>%
...
nb_no_calls_to_eval <- nrow(corpus) - n_distinct(select(packages_at_least_one_call_site, eval_source))
...
```{r nb-eval-calls}
# run = package?
eval_calls_per_package <- own_eval_calls %>%
...
med_size <- weighted.median(own_eval_calls$expr_expression_length, w = own_eval_calls$nb_ev_calls, na.rm = TRUE)
max_size <- max(own_eval_calls$expr_expression_length, na.rm = TRUE)
average_size <- weighted.mean(own_eval_calls$expr_expression_length, na.rm = TRUE, w = own_eval_calls$nb_ev_calls)

sizes_prop <- own_eval_calls %>%
...
```{r loops}
eval_calls_exercised <- eval_calls %>%
  group_by(file) %>%
