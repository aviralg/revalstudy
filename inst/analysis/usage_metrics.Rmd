...
```{r}
E_raw %>% select(package, eval_function, nb_ev_calls,
                 eval_call_srcref, file, eval_source, 
                 expr_expression_length,
                 direct_interpreter_eval)-> E
C_raw -> C
corpus_size <- nrow(C)
```
...
nb_calls <- (E %>% count(wt=nb_ev_calls))$n
eval_calls <- E %>% filter(eval_function=="eval") %>% count(wt=nb_ev_calls)
evalq_calls <- E %>% filter(eval_function=="evalq") %>% count(wt=nb_ev_calls)
eval.parent_calls <- E %>% filter(eval_function=="eval.parent") %>%
  count(wt = nb_ev_calls)
local_calls <- E %>% filter(eval_function=="local") %>% count(wt = nb_ev_calls)
eval_at_least_once <- E %>% count(package, wt=nb_ev_calls) 
...
r("nb package eval at least once", nrow(eval_at_least_once))
``` 
...
call_sites_per_package <- E %>%
  ...
  summarize(nb_call_sites = n_distinct(eval_call_srcref, na.rm = TRUE))
...
max_packages <- packages_at_least_one_call_site %>%
  ...
  select(eval_source, nb_call_sites)
nb_no_calls_to_eval <- corpus_size - 
  n_distinct(select(packages_at_least_one_call_site, eval_source))
...
E %>%
  group_by(eval_source) %>%
  mutate(nb_call_sites = n_distinct(eval_call_srcref, na.rm = TRUE)) %>%
...
eval_calls_per_package <- E %>%
  count(eval_source, wt = nb_ev_calls) %>%
  left_join(runs_package, by = "eval_source") %>%
  mutate(n = n / nb_runs)
...
med_size <- weighted.median(E$expr_expression_length, w = E$nb_ev_calls, na.rm = TRUE)
max_size <- max(E$expr_expression_length, na.rm = TRUE)
average_size <- weighted.mean(E$expr_expression_length, na.rm = TRUE, w = E$nb_ev_calls)

sizes_prop <- E %>%
  count(expr_expression_length, wt = nb_ev_calls) %>%
...
nb_events_in_eval <- E %>% summarise(nb_events = sum(direct_interpreter_eval))

max_events_in_eval <- E %>% summarize(maxi = max(direct_interpreter_eval))

less_100_events <- E %>% filter(direct_interpreter_eval <= 100)
...
E %>%
  filter(direct_interpreter_eval > 0) %>%
...
```{r loops}
eval_calls_exercised <- E %>% group_by(file) %>%
  count(eval_call_srcref, wt = nb_ev_calls)

eval_calls_exercised %>% ggplot(aes(n)) +
  geom_histogram() + scale_x_log10(labels = scales::comma) +
