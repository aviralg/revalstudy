...
docker on Jan's mamchine. This should be removed.
...
```{r read,echo=F,cache=F}
read_fst(eval_calls_path) %>% as_tibble() -> E_raw
read_fst(corpus_path) %>% as_tibble() -> C_raw
#read_csv(paste0(base_dir, "/run/package-evals-traced.9/package-eval-static.csv")) -> P
...
# Eval Calls Overiew
The corpus has `r corpus_size` packages, and the data set has 
`r nrow(E)` distinct calls to the `eval` and its variants.

```{r eval-calls-overview,echo=F}
nb_calls <- E %>% 
  count(wt = duplicates) %>%
  .$n
eval_calls <- E %>%
...
```{r call-sites-violin-plot, echo=F}
E %>% 
  select(ev_package, ev_variant, src_ref) %>%
  group_by(ev_package, ev_variant) %>%
  summarize(sites=n_distinct(src_ref)) %>%
