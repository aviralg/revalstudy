---
title: "Usage metrics"
author: "Pierre Donat-Bouillud"
output: html_document
date: "`r Sys.Date()`"
editor_options: 
  chunk_output_type: console
params:
  base_dir: /var/lib/R/project-evalR
  calls_packages_path: !r paste0(params$base_dir, "/run/package-evals-traced.5/summarized-packages.fst")
  corpus_path: !r paste0(params$base_dir, "/revalstudy/inst/data/corpus.fst")
---

```{r setup, include=FALSE}
knitr::knit_hooks$set(time_it = local({
  now <- NULL
  function(before, options) {
    if (before) {
      # record the current time before each chunk
      now <<- Sys.time()
    } else {
      # calculate the time difference after a chunk
      res <- difftime(Sys.time(), now)
      # return a character string to show the time
      paste("Time for this code chunk to run:", res, units(res))
    }
  }
}))

knitr::opts_chunk$set(echo = TRUE,
                      fig.retina = 2,
                      fig.width = 10,
                      cache.lazy = FALSE,
                      time_it = TRUE)


now <- Sys.time() 
library(tidyverse)
library(stringr)
library(fst)
library(fs)


source("inc/paths.R")
source("inc/latextags.R")
source("inc/setup.R")
theme_set(theme_minimal())
create_tags(path(TAGS_DIR, "enrichDataset.tex"), prefix="", default=TRUE)
```


```{r file-info, echo=FALSE}
file_info(params$calls_packages_path)
```

```{r read-data}
eval_calls <- read_fst(params$calls_packages_path) %>% as_tibble()
```

```{r read_corpus}
corpus <- read_fst(params$corpus_path) %>% as_tibble()
```


```{r eval-calls-overview}
own_eval_calls <- eval_calls %>% filter(eval_source == package) # package column is wrong currently
nb_eval_calls <- own_eval_calls %>% count(wt = nb_ev_calls) %>% select(n)

nb_eval_pres_calls <- own_eval_calls %>% filter(eval_function == "eval") %>% count(wt = nb_ev_calls) %>% select(n)
nb_evalq_calls <- own_eval_calls %>% filter(eval_function == "evalq") %>% count(wt = nb_ev_calls) %>% select(n)
nb_eval.parent_calls <- own_eval_calls %>% filter(eval_function == "eval.parent") %>% count(wt = nb_ev_calls) %>% select(n)
nb_local_calls <- own_eval_calls %>% filter(eval_function == "local") %>% count(wt = nb_ev_calls) %>% select(n)

eval_at_least_once <- own_eval_calls %>% count(package, wt = nb_ev_calls) %>% nrow()

r("nb eval and variant calls", nb_eval_calls)
r("nb eval calls", nb_eval_pres_calls)
r("nb evalq calls", nb_evalq_calls)
r("nb eval parent calls", nb_eval.parent_calls)
r("nb local calls", nb_local_calls)
r("nb package eval at least once", eval_at_least_once)
``` 


# Number of eval call sites

```{r nb-eval-call-sites}
nb_ev_call_sites <- eval_calls %>% select(eval_call_srcref) %>% n_distinct(na.rm = TRUE)

call_sites_per_package <- eval_calls %>% group_by(eval_source) %>% mutate(nb_call_sites = n_distinct(eval_call_srcref, na.rm = TRUE))

nb_packages_one_call_site <- call_sites_per_package %>% filter(nb_call_sites == 1) %>% nrow()

packages_at_least_one_call_site <- call_sites_per_package %>% filter(nb_call_sites >= 1)

median_call_sites <- packages_at_least_one_call_site %>% summarise(med = median(nb_call_sites, na.rm = TRUE))
max_packages <- packages_at_least_one_call_site %>% filter(nb_call_site == max(nb_call_sites, na.rm = TRUE)) %>% select(eval_source, nb_call_site)

nb_no_calls_to_eval <- nrow(corpus) - nrow(packages_at_least_one_call_site)

r("nb eval call sites", nb_ev_call_sites)
r("packages no calls to eval", nb_no_calls_to_eval)
```


```{r call-sites-violin-plot}
call_sites_per_package %>%
 ggplot(aes(x = eval_source, y = nb_call_sites, fill = eval_function), na.rm = TRUE) +
  geom_violin(trim = FALSE, width=1.7)+
  geom_boxplot(data = slice_sample(nb_ev_call_sites, n = 100000), width=.03) + # too slow otherwise
  #scale_y_log10(labels = scales::comma) +
  labs(y="Call sites per package") +
  theme(
    legend.position=c(0.5, 0.85),
    legend.box.background = element_rect(fill="white", size=0.1),
    legend.box="horizontal",
    axis.title.x=element_blank()
  )

ggsave(path(PLOT_DIR, "call_sites_per_packages.pdf"))
```


# Numbers of calls to eval

This is the number of eval calls which have a call site in each package.

```{r nb-eval-calls}
# run = package?
eval_calls_per_package <- own_eval_calls %>% count(eval_source, wt=nb_ev_calls) 

average_eval_calls <- eval_calls_per_package %>% filter(n > 0) %>% summarise(, average = mean(n)) %>% .["average"]
max_eval_calls <- eval_calls_per_package %>% filter(n = max(n))

r("average eval calls per package", average_eval_calls) # average on packages with at least one eval call
r("max eval calls", max_eval_calls$n)
r("max eval call package", max_eval_calls$eval_source)
```


```{r eval-calls-violin-plot}
call_sites_per_package %>%
 ggplot(aes(x = eval_source, y = n), na.rm = TRUE) +
  geom_violin(trim = FALSE, width=1.7)+
  geom_boxplot(data = slice_sample(nb_ev_call_sites, n = 100000), width=.03) + # too slow otherwise
  #scale_y_log10(labels = scales::comma) +
  labs(y="Eval calls per package") +
  theme(
    legend.position=c(0.5, 0.85),
    legend.box.background = element_rect(fill="white", size=0.1),
    legend.box="horizontal",
    axis.title.x=element_blank()
  )

ggsave(path(PLOT_DIR, "call_sites_per_packages.pdf"))
```

```{r}
nb_eval_calls_tot <- eval_calls %>% count(wt = nb_ev_calls) %>% select(n)
```

If we don't exclude dependent calls, there are `r nb_eval_calls_tot` eval calls, so the proper eval calls are `r percent(nb_eval_calls, nb_eval_calls_tot)` of the total.



# Amount of code loaded by eval

```{r amout-of-code}

med_size <- own_eval_calls %>% summarise(med = weighted.median(expr_expression_length, na.rm = TRUE, w = nb_ev_calls))
max_size <- own_eval_calls %>% summarize(m = max(expr_expression_length, na.rm = TRUE))
average_size <- own_eval_calls %>% summarize(average = weithed.mean(expr_expression_length, na.rm = TRUE, w = nb_eval_calls))

sizes_prop <- own_eval_calls %>% count(expr_expression_size, wt = nb_ev_calls) %>%
                      mutate(proportion = n / sum(n)) %>% 
                      arrange(expr_expression_size) %>% 
                      mutate(cumulative = cumsum(proportion))
  

nb_nodes_many <- sizes_prop %>% filter(cumulative >= 0.95) %>% .[1,] 

r("size nine five", nb_nodes_many$expr_expression_size)
r("size nine five exact percent", percent(nb_nodes_many$cumulative))

```



# Amount of computations via eval


```{r amount_computations}
nb_events_in_eval <- own_eval_calls %>% summarise(nb_events = sum(direct_interpreter_eval))

max_events_in_eval <- own_eval_calls %>% summarize(maxi = max(direct_interpreter_eval))

less_100_events <- own_eval_calls %>% filter(direct_interpreter_eval <= 100)
nb_less_100_events <- count(less_100_events, wt = nb_ev_calls)$n

r("nb events eval", nb_events_in_eval$nb_events)
r("maxi events eval", max_events_in_eval$maxi)
r("small events eval", percent(count(nb_less_100_events), nb_eval_calls))
```

```{r nb-events-violin-plot}
own_eval_calls %>%
 ggplot(aes(x = eval_source, y = direct_interpreter_eval), na.rm = TRUE) +
  geom_violin(trim = FALSE, width=1.7)+
  geom_boxplot(data = slice_sample(direct_interpreter_eval, n = 100000), width=.03) + # too slow otherwise
  #scale_y_log10(labels = scales::comma) +
  labs(y="Number of events in eval per package") +
  theme(
    legend.position=c(0.5, 0.85),
    legend.box.background = element_rect(fill="white", size=0.1),
    legend.box="horizontal",
    axis.title.x=element_blank()
  )

ggsave(path(PLOT_DIR, "nb_events_per_packages.pdf"))
```

# The case of core libraries
