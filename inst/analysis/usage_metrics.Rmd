...
params:
  base_dir: /var/lib/R/project-evalR/run/package-evals-traced.10/
  calls_packages_path: summarized-packages.fst
  corpus_path: corpus.fst
  static_evals_path: package-evals-static.csv
---

The setup chunk has a variable `jan` with setting to run the script on
docker on Jan's machine. This should be removed.
...
if (jan) {
  base_dir <- "/mnt/code/eval/run/package-evals-traced.9/"
}
...
The corpus has `r corpus_size` packages, and the data set has `r nrow(E)` 
distinct calls to the `eval` and its variants. There are 
`r length(unique(E$ev_package))` corpus packages with `eval` calls (note that
we may not trigger all `eval`s in a package due to spotty covereage).
...
TBL <- overview_table(
  ...
  r("nb eval calls", eval_calls),
  r("nb evalq calls", evalq_calls),
  r("nb eval parent calls", eval.parent_calls),
  r("nb local calls", local_calls)
)
``` 
...
group_tags <- cut(eval_calls_table$n,
                  breaks = breaks,
                  include.lowest = TRUE)

ev_summary <- summary(group_tags)
...
TBL <- overview_table(
  ...
  r("at least one call site", sites_per_package),
...
And now, by `eval` variant:

```{r exercised-call-site-per-variant}

static_eval <- P %>% filter(call_fun_name == "eval") %>% nrow()
static_evalq <- P %>% filter(call_fun_name == "evalq") %>% nrow()
static_eval.parent <- P %>% filter(call_fun_name == "eval.parent") %>% nrow()
static_local <- P %>% filter(call_fun_name == "local") %>% nrow()
...
trig_eval <- sites_per_variant %>% filter(ev_variant == "eval") %>% .$n
trig_evalq <- sites_per_variant %>% filter(ev_variant == "evalq") %>% .$n
trig_eval.parent <- sites_per_variant %>% filter(ev_variant == "eval.parent") %>% .$n
trig_local <- sites_per_variant %>% filter(ev_variant == "local") %>% .$n

r("static eval", static_eval)
r("static evalq", static_evalq)
r("static eval.parent", static_eval.parent)
r("static local", static_local)

r("triggered eval", trig_eval)
r("triggered evalq", trig_evalq)
r("triggered eval.parent", trig_eval.parent)
r("triggered local", trig_local)

r("eval percent call site", ratio(trig_eval, static_eval))
r("evalq percent call site", ratio(trig_evalq, static_evalq))
r("eval.parent percent call site", ratio(trig_eval.parent, static_eval.parent))
r("local percent call site", ratio(trig_local, static_local))
```
