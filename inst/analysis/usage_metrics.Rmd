...
read_csv(eval_static_path) %>% semi_join(C_raw, by = "package") -> E_static
```
...
with all columns. E_static corresponds to static information about the call sites.
...
nb_static_call_sites <- nrow(E_static)

static_call_sites_per_package <- E_static %>%
  count(package, sort = TRUE)

static_at_least_one_call_site <- static_call_sites_per_package %>%
  filter(n >= 1)

r("nb static call sites", nb_static_call_sites)
r("call sites percent", ratio(nb_ev_call_sites, nb_static_call_sites))
r("nb static at least one call site", nrow(static_at_least_one_call_site))
r("static at least one call site percent", ratio(nrow(packages_at_least_one_call_site), nrow(static_at_least_one_call_site)))
```
