...
# TODO
PACKAGE_COVERAGE_LOG <- "/home/rstudio/evalR/run/package-coverage/parallel.csv"
```

## Description

The aim of this notebook is to analyze the corpus of R packages.

## Loading data

This notebook requires the following data files:

- `r PACKAGE_COVERAGE_LOG` - package coverage log
- `r PACKAGE_COVERAGE` - package coverage

```{r load data - packages, include=FALSE}
coverage_log <- read_runr_log(PACKAGE_COVERAGE_LOG)
```

```{r}
failed_coverage <- filter(coverage_log, exitval != 0)
```

```{r}
failed_coverage %>%
  mutate(
    run_path=show_url(run_path)
  ) %>%
