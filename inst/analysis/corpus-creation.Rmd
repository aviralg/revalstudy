...
blacklist <- tribble(
  ~package,
  "ggm",       # requires bioc
  "future",    # does not work with OMP thread setting
  "shinyBS",   # runs shiny server as part of its tests
  "spatstat",  # takes too long
  "survival"   # takes too long
)
```
...
cran_packages_raw <- installed.packages()
cran_packages <- as_tibble(cran_packages_raw) %>%
  select(package=Package, version=Version) %>%
  anti_join(blacklist, by="package") %>%
  anti_join(BASE_PACKAGES, by="package")
```
...
```{r get last month most downloaded packages, eval=F}
pkgs_top_downloaded <-
  cranlogs::cran_top_downloads("last-month", 100)
```

```{r compute missing packages from the most downloaded ones, eval=F}
...
write_lines(sort(pkgs_top_revdeps$package), CORPUS_FILE)
write_csv(pkgs_revdeps, CORPUS_REVDEPS)
write_lines(sort(unique(pkgs_revdeps_raw$revdep)), CORPUS_REVDEPS_FILE)
```
