...
Let's remove the duplicate ones (but keep their numbers):
...
```{r}
eval_calls_base <- eval_calls_base %>% count(across(c(-eval_call_id, -starts_with("caller_stack"))), name = "nb_ev_calls") 
eval_calls_dedup <- eval_calls %>% count(across(c(-eval_call_id)), name = "nb_eval_calls")
eval_calls_dedup_more <- eval_calls %>% count(across(c(-eval_call_id, -starts_with("caller_stack"))), name = "nb_ev_calls") 
```

```{r}
eval_calls_dedup_more <- eval_calls_dedup_more %>% mutate(parse_args = map(expr_parsed_expression, function(e) { if(!is.na(e) && str_starts(e, "(parse|str2lang|str2expression)\\("))   extract_args_parse(e) else list() }))  %>% unnest_wider(parse_args)
```

```{r}
eval_calls_dedup_more <- eval_calls_dedup_more %>% mutate(eval_call_package = map2_chr(eval_call_srcref, file, extract_package_name)) 
```

```{r}
eval_calls_dedup_more <- eval_calls_dedup_more %>% 
    resolve_sexp_name(expr_expression_type) %>%
    resolve_sexp_name(expr_resolved_type) %>%
    resolve_sexp_name(enclos_type) %>%
    resolve_sexp_name(envir_type)
...
```{r}
eval_calls <- eval_calls %>% select(-ends_with("_type")) %>% left_join(eval_calls_dedup_more)
eval_calls_dedup <- eval_calls_dedup %>% select(-ends_with("_type"))%>% left_join(eval_calls_dedup_more)
```


```{r write_back}
write_fst(eval_calls_dedup, paste0(params$data_dirpath, "deduplicated_calls.fst"))

write_fst(eval_calls_dedup_more, paste0(params$data_dirpath, "deduplicated_more_calls.fst"))
