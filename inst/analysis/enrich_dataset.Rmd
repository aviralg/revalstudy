...
eval_calls_dedup_more <- eval_calls %>% count(across(c(-eval_call_id, -starts_with("caller_stack"))), name = "nb_eval_calls") 
```

```{r}
eval_calls <- eval_calls_dedup_more %>% mutate(parse_args = if_else(is.na(expr_parsed_expression), NA, map_chr(expr_parsed_expression, extract_args_parse)))
```
