...
Let's remove the duplicate ones:

```{r}
eval_calls_base <- eval_calls_base %>% select(-eval_call_id, -starts_with("caller_stack")) %>% distinct()

eval_calls <- eval_calls_base %>% select(-eval_call_id) %>% distinct()
```
...
```{r}
# We are doing some work once again! Parsing what was deparsed previously...
...
write_dst(eval_calls_base, paste0(params$data_dirpath, "base_calls.fst"))
```
