...
ast_sizes %>%
  ggplot(aes(x = eval_source_type, y = expr_resolved_nodes, fill = eval_source_type, weight = nb_ev_calls), na.rm = TRUE) +
...
```{r}
eval_calls %>% semi_join(expr_resolved_polymorphism %>% filter(polym_degree == max(polym_degree)), by ="eval_call_srcref")
```
