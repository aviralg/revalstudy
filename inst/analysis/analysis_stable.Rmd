...
theme_update(text = element_text(rel(1.5)))
...
eval_calls_raw <- bind_rows(eval_calls_packages, eval_call_core)
```
...
classified_envir %>% ggplot() +
  geom_bar(aes(x = write_env, weight = nb_ev_calls, group = dataset)) +
  labs(x = "Environment write")
```

```{r read_env}

classified_envir %>% ggplot() +
  geom_bar(aes(x = read_env, weight = nb_ev_calls, group = dataset)) +
  labs(x = "Environment reads")
```
