...
expr_call_arg <- eval_calls %>% filter(expr_expression_type == "LANGSXP") %>% 
  mutate(function_expr = map_chr(expr_expression, function_name))
```

The following top level functions are used:

```{r top_level_function_expr}
expr_call_arg %>% count(function_expr) %>% arrange(desc(n))
```
...
group_expr_functions <- expr_call_arg %>%
                            mutate(function_expr2 = if_else(str_starts(function_expr, fixed("(function(")), "anonymous", function_expr))
...
expr_resolved_call_arg <- eval_calls %>% filter(expr_resolved_type == "LANGSXP") %>% 
  mutate(function_expr = map_chr(expr_resolved, function_name))
```

The following top level functions are used:

```{r top_level_function_resolved_expr}
expr_resolved_call_arg %>% count(function_expr) %>% arrange(desc(n))
```
