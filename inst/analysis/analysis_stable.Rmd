...
There are `r nrow(parsed_base)` `eval` calls with `parse` in *base*, which is `r nrow(parsed_base) / nrow(filter(eval_calls, eval_call_package == "base"))`%. 
...
We look at the first argument of `eval`, the `expr` argument. It is the expression to be evaluated. It can be of different types, mainly usual numeric types, vectors, strings, and language types: calls, symbols, promise, bytecode and expressions.
...
Expressions of types other than `call`,  `expression`, `name`, `bytecode` and `promise` are just returned unchanged.


```{r not_unchanged_eval}
changed_evals <- eval_calls %>% mutate(changed =  fct_collapse(expr_expression_type, lang_types = c("LANGSXP", "EXPRSXP", "SYMSXP", "PROMSXP", "BCODESXP"), other_level = "other_types")) %>% count(changed) %>% mutate(per_cent = n / nb_eval_calls * 100 ) 
```
...
### `expr` argument when it is a call

```{r expr_call_arg}
expr_call_arg <- eval_calls %>% filter(expr_expression_type == "LANGSXP")
```

The following top level functions are used:

```{r top_level_function_expr}
expr_call_arg %>% count(expr_expression_function, sort = TRUE) 
```
...
```{r group_expr_functions}
group_expr_functions <- expr_call_arg %>%
                            #mutate(function_expr2 = if_else(str_starts(expr_expression_function, fixed("(function(")), "anonymous", expr_expression_function))
                            mutate(function_expr2 = groupify_function(expr_expression_function)) %>%
  
group_expr_functions <-  group_expr_functions %>% 
                              ...
                              expressions = c("as.list", "expression", "as.name", "as.expression", "list", "as.data.frame", "as.character", "{", "modify_lang"),
                              ...
                              function_meta = c("sys.calls", "function", "body"),
...
```{r}
group_expr_functions %>% count(group_function_expr) %>% arrange(desc(n)) %>% knitr::kable()
```

```{r}
group_expr_functions %>% ggplot() +
  geom_bar(aes(x = fct_rev(fct_infreq(group_function_expr)))) +
  scale_y_sqrt() +
  coord_flip() +
...
```{r arity_expr_expression}
expr_call_arg %>% count(expr_expression_args_num, sort = TRUE) %>% knitr::kable()
```

```{r arity_expr_expression_plot}
expr_call_arg %>% ggplot() + 
  geom_bar(aes(x = expr_expression_args_num)) +
  scale_y_sqrt() +
  coord_flip() + 
...
```{r arity_expr_expression_plot_per_eval}
expr_call_arg %>% ggplot() + 
  geom_bar(aes(x = expr_expression_args_num)) +
  scale_y_sqrt() +
  coord_flip() + 
...
We can do the same for resolved expressions:

```{r types_resolved}
per_expr_resolved_type <- eval_calls %>% count(expr_resolved_type)
```
...
changed_resolved_evals <- eval_calls %>% mutate(changed =  fct_collapse(expr_resolved_type, lang_types = c("LANGSXP", "EXPRSXP", "SYMSXP", "PROMSXP", "BCODESXP"), other_level = "other_types")) %>% count(changed) %>% mutate(per_cent = n / nb_eval_calls * 100 ) 
```
...
expr_resolved_call_arg %>% count(expr_resolved_function) %>% arrange(desc(n))
```

```{r group_expr_resolved_functions}
group_expr_resolved_functions <- expr_resolved_call_arg %>%
                            #mutate(function_expr2 = if_else(str_starts(expr_resolved_function, fixed("(function(")), "anonymous", expr_resolved_function)) %>%
...
```{r}
group_expr_resolved_functions %>% count(group_function_expr) %>% arrange(desc(n)) %>% knitr::kable()
```
...
expr_resolved_call_arg %>% ggplot() + 
  geom_bar(aes(x = expr_resolved_args_num)) +
  scale_y_sqrt() +
  coord_flip() + 
  labs(title = "Arity of calls", x = "arity", y = "n")
