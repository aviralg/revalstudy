...
r("percent parsed evals",  ratio(nb_parsed_evals, nb_eval_calls))
```
...
We also look to the occurences of `paste` and `str_c` in the `parse` argument. It is an indication that meta-pogramming happens.

```{r parse_meta_prog}
meta_parsed <- parsed_texts %>% filter(str_detect(expr_parsed_expression, "paste|str_c"))

nb_meta_parsed <- meta_parsed %>% count(wt = nb_ev_calls)
nb_meta_parsed_call_sites <- nb_eval_call_sites(meta_parsed)

r("paste parsed evals", ratio(nb_meta_parsed, nb_parsed_evals))
