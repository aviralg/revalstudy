...
```{r env_assign}
eval_calls_env_assign <- eval_calls %>% filter(!is.na(expr_resolved)) %>% mutate(env_assign = pblapply(expr_resolved, is_assign_str, cl = parallel::detectCores() - 2))

#eval_calls_env_assign2 <-  eval_calls_env_assign %>% filter(is.logical(env_assign)) %>% hoist("env_assign", env_assign = 1)
```
