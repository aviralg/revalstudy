...
all_data <- bind_rows(core_data, packages_data, kaggle_data) %>% as_tibble()
all_data <-
    all_data %>%
...
# TODO - we need to summarize the data better
# all paths must start with this
stopifnot(nrow(filter(all_data, !startsWith(file, "../../run/"))) == 0)
```

```{r}
cran_programs <- read_fst(CRAN_PROGRAMS_FILE) %>% 
  as_tibble() %>%
  filter(success) %>%
  select(file, corpus)

kaggle_programs <- read_fst(KAGGLE_PROGRAMS_FILE) %>% 
  as_tibble() %>%
  filter(success) %>%
  select(file, corpus)


all_programs <- bind_rows(cran_programs, kaggle_programs)
```

How many programs are missing in the summarized data

```{r}
anti_join(all_programs, all_data, by="file")
```
...
all_data_summary <- 
  ...
  right_join(all_programs_cod, by=c("file", "corpus", "core_own_dependent")) %>%
...
#   summarize(
#     total_eval_calls = sum(eval_calls),
...
map2_dfr(vars$corpus, vars$cod, function(crps, cod) {
  print(filter(core_own_dependent, corpus==crps, core_own_dependent==cod))
