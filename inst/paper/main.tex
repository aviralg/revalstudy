...
expressions which are reduced to values by \eval. % Furthermore, the proportion
of values % passed to core \eval call is \CoreEvalValueInputPerc compared to a
significantly % higher \PackageEvalValueInputPerc for package \eval calls. To
...
for 80\% of the calls. The most frequently occurring call is to the
...
The most frequent ten expressions to eval calls from core R contribute to
85.2\% of all \eval calls.
\begin{table*}[tb]
  \centering
  \begin{tabular}{l|rr} \hline
    ...
    \c{local(\{info <- loadingNamespaceInfo(...} & 1008632 &       14.9\%\\
    ...
    \c{\<NULL\>}& 75330  &      1.1\% \\
    \c{\<Integer Vector\>}&                 66279&       1\%\\\hline
  \end{tabular}
