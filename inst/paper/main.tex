...
analysis to record the calls to evals.
...
The corpus for this study includes R programs extracted from popular R packages
as well as complete user-written data-analysis scripts. In this section we
describe the methodology used to collect them.

\subsection{CRAN Packages}\label{sec:corpus}
...
We endeavored  to write these functions without \eval. \c{match.arg} appears in 60.1\% of the eval calls. It is used to get the arguments, possibly abbreviated, of a function. With one argument, as \c{match.arg(arg)}, it uses \c{eval} to infer them from a parent function's formal arguments. We can rewrite it using its version with two arguments, \c{match.arg(arg, choices)} where \c{choices} are the possible formal arguments of the function, as shown in Listing~\ref{code:matcharg}.
