...
percents and the y-axis gives the number of call sites to \eval in log
...
\summaryrnd{CorpusExamplesCode}\footnote{This notation represents $sum$
  ($min$ / $mean$ / $max$)} lines of code, and \CorpusVignettesProgramsRnd
vignettes.%%% with \summaryrnd{CorpusVignettesCode} lines of code.
...
kernels with \summaryrnd{CorpusFinishedKaggleCode} lines of R code.
...
CRAN packages. 61 packages have a single call site, and one package has 142
call sites of \eval. 60 packages have no calls to \eval. The core packages
%%%% FIX THESE NUMBERS
have \miss{41} callsites of eval and Kaggle has none. These numbers are
...
calls. Sizes in core are larger than in packages. Most large AST originate
from library \c{VGAM}, which is used for statistical modelling. It uses
...
\EventsMaxCount.  However majority of \eval calls perform up to
...
%%
%% \end{lstlisting}
%%$ <- to fix emacs fontification issues
...
patterns. The patterns were detected using simple regular expressions on the
string passed to \eval. In the case of R look at the AST passed to evals.

\missingTable[JS evals in R]

For the rest of this section we show some of the common use of \eval in R that
we have seen by looking at 20 randomly selected packages:
...
\subsubsection{Evaluation using Data Fields} A common use of \eval is to
  ...
  locked for modification and require explicit unlocking using \unlockBinding
  function. However, directly using this function triggers a
  ...
  dependencies.
  % \AllValueInputEvalCallPerc migth hit on a wong usage
