...
\EventsMinRangeC--\EventsMaxRangeC & \EventsCoreEvalPercC & \EventsPackageEvalPercC \\
...
However majority of \eval calls perform up to \EventsMaxRangeA events.
...
after being resolved, must be evaluated. By default, it is equal to \c{parent.frame()}, which is the parent environment of the \c{eval} call. This default argument happens in \miss{XX\%} of the eval calls.
...
As shown in Figure~\ref{fig:environments}, \eval in R is often performed in the global environment.  It is also used a lot to evaluate in the context of a dataframe (\emph{list} environment).
