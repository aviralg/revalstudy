...
We recorded \NbEvalAndVariantCallsRnd calls to \eval and its variants (\NbEvalCallsRnd
for \eval, \NbEvalqCallsRnd for \c{evalq}, \NbEvalParentCallsRnd for \c{eval.parent} and \NbLocalCallsRnd for
\c{local}).
...
Figure~\ref{fig:callSitesPerPackage} shows a violin plot of the distribution of call sites to
\eval and its three variants. For packages that have call sites, the
median is \NbMedianCallSites call sites. \NbOneCallSite{} packages have a
single call site, and one package, \emph{\MaxCallSitesPackage}, has
\MaxCallSites{} call sites. \PackagesNoCallsToEval{} packages have
no calls sites to \eval. These numbers are under-approximations as \eval may
