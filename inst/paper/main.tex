...
%\noindent
%\begin{compactitem}[$-$]
  % \item \emph{Library}: in JavaScript, the library pattern captures asynchronous
  % loading of external external function definitions. In R we have seen only
  % \PatternLibRnd such calls (\PatternLibRatio) mostly coming from source loading
  % functions (\ie \c{source} and \c{sys.source}). Most of the external function
  % definitions in R come from packages and R makes it very easy for users to
  % install packages from its centralized repository.
...
for (i in names(opts)) eval(parse(text=paste0("options(",i,"=",opts[i],")")))
  ...
  For example, the \c{PerfromanceAnalytics} package contains a function \c{chart.QQPlot} that uses \eval to resolve a string into function and anther to call it and assign its results into a variable:
  \begin{lstlisting}
