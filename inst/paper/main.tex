...
\mypara{Sandbox.} The combination of \eval and first class
...
Recording calls - this one is more on the misuse than doing something useful. I
really wonder why they do not use \c{do.call(write.table, Call)} (except that it
will create additional frames).
\begin{lstlisting}
write.csv <- function (...)
{
  Call <- match.call(expand.dots = TRUE)
...
In vcrpart package they use this a lot (following is just one instance):

\begin{lstlisting}
vcrpart::tvcglm <- function(formula, data, family,
...
The examples above are related to the use of \c{match.call} whose documentation talks about these use cases:

\begin{verbatim}
‘match.call’ is most commonly used in two circumstances:

        • To record the call for later re-use: for example most
          model-fitting functions record the call as element ‘call’ of
          the list they return.  Here the default ‘expand.dots = TRUE’
          is appropriate.

        • To pass most of the call to another function, often
          ‘model.frame’.  Here the common idiom is that ‘expand.dots =
          FALSE’ is used, and the ‘...’ element of the matched call is
          removed.  An alternative is to explicitly select the
          arguments to be passed on, as is done in ‘lm’.
\end{verbatim}


creating and evaling strings - here they assemble the call to plot function.
plot is s3 generic which takes ridiculous amount of parameters and in addition
their plot function also takes parameters. This code could be nicely refactored
using do.call.
\begin{lstlisting}
wsbackfit::plot.sback <- function(x, composed = TRUE, ask = TRUE, select = NULL, ...) {
 dots <- list(...)
 grph.opt <- names(dots)
...
 stub <- paste(
  ifelse("xlab" %in% grph.opt, "", paste(", xlab = \"", var, "\"",sep = "")),
  ifelse("ylab" %in% grph.opt, "", paste(", ylab = \"", p.functions[i], "\"", sep = "")),
  ifelse("main" %in% grph.opt, "", main.aux),
  ifelse("type" %in% grph.opt, "", ", type = \"l\""),
  ifelse("ylim" %in% grph.opt, "", paste(", ylim = c(", min.ylim,",", max.ylim,")", sep = "")), ",...)",
  sep = ""
 )
