...
% Furthermore, the proportion of values
% passed to core \eval call is \CoreEvalValueInputPerc compared to a significantly
% higher \PackageEvalValueInputPerc for package \eval calls.
To investigate this, we look at the type of expressions accepted by \eval
callsites in our corpus. We find that only \AllExpressionInputEvalSitePerc call
sites exclusively receive expressions as arguments across all program runs. The
remaining \AllPolymorphicInputEvalSitePerc call sites don't always accept expressions
as inputs. The top three of these call sites contribute to over
...
%%Figure~\cite{fig:eval-expr-type} by the type of input and
...
%%as inputs. In fact, \PackageEnvironmentEvalPerc of inputs to package evals are
%%environments.
%%
%%Now, we turn our attention to the expressions passed as input to \eval.
...
%	\caption{The environments in which the expression is evaluated. \c{NA} corresponds to the default environment.} \label{fig:environments}
%\end{figure*}
