...
with \c{match.call}. What \c{match.call} does is step up the call
stack and capture the invocation of the currently executing function
and return it as an unevaluated expression. The general pattern here
is that a call to some function \c{f} is transformed to a call to
another function \c{g} with some arguments retained and others
modified. An illustration taken from the \c{vcpart} package shows a
function \c{tvcglm} that is translated to the \c{tvcm} function but
with two modifications to the argument list, argument \c{control}
can't be missing and argument \c{fit} is forced to have value \c{glm}.
The function ends with a call to \c{eval.parent} to ensure that the
...
\begin{lstlisting}
 plot.sback <- function(x,...) {
   ...
   ...
\end{lstlisting}
The \parse and \eval combination is used by the the \source function
in the base library to load R code from a file in the current
workspace.
