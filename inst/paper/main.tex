...
choose popular packages which in general should be of a better quality and
should have better code coverage. The resulting set has \summary{CorpusRevdeps}
reverse dependencies (min / mean / max) and consists of \CorpusRCodeRnd lines of
...
number of reverse dependencies in log scale. Dotted lines indicate means. Large
(over \CorpusLargePackageTresholdRnd lines of code) and popular (in
\CorpusMostDownloadedTreshold most downloaded) packages are annotated.
...
\emph{tests}, there are: \emph{examples} and \emph{vignettes}. Examples are R
code snippets extracted from package documentation into scripts file. Vignettes
are long-form guides into package functionality written using
...
\CorpusPackageProgramsCodeRnd lines of R code. Most programs are examples
(\CorpusExamplesPrograms) with \CorpusExamplesCodeRnd lines of code, followed by
tests (\CorpusTestsPrograms) with \CorpusTestsCodeRnd lines of code and
vignettes (\CorpusVignettesPrograms) with \CorpusVignettesCodeRnd lines of code.

Running this code results on average in \CorpusMeanExprCoverage package code
coverage. The code coverage was computed for each package using
Covr\footnote{\url{ https://github.com/r-lib/covr}}, the R code coverage tool.
