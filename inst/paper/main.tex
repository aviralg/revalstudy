...
\c{x}, the expression corresponding to that argument, i.e., \c{a+b}.
The general form of the function takes two arguments, the second
argument is an object that can be used as an environment. This can be
either a list of named values, a data frame or an actual environment.
\begin{lstlisting}
  > environment()
  # <environment: R_GlobalEnv>
\end{lstlisting}
The above shows how to access the current environment, this can belong
to the current function or, as here, the top level. Environments are
...
\item \c{env$v <- 2} and \c{assign("v", 2, envir = env)} write 2 in
  \c{v}. If it did not exist, \c{v} is created in \c{env}.
\end{itemize}
...
\end{lstlisting}
The above code show simplified implementation for the variants of
\eval. The \c{evalq} form simply quotes the argument to prevent it
from being evaluated in the current environment. The
\c{eval.parent(e,n)} form evaluates \c{e} in the environment of the
  {\tt n}-th caller up the call stack. Finally \c{local} evaluates an
  expression in a new environment to avoid polluting the current
  environment.

%% Taking an expression rather
...
%% access to the code of a promise as an expression.}
