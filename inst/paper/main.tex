...
We have assembled a corpus of \CorpusPackages R packages and
\CorpusFinishedKaggle user-written data-analysis scripts. Together it contains
\CorpusAllCodeRnd lines of source code\footnote{Excluding comments and blank lines using \url{github.com/AlDanial/cloc}}.

\subsection{CRAN Packages}

We have selected the TOP \CorpusPackages packages based on the number of reverse
dependencies, \ie how many packages depend on them\footnote{extracted from
  package metadata using builtin function}. Using the number of reverse
dependencies helps us to choose popular packages which in general should be of a
better quality and should have better code coverage. The resulting set consists
of \CorpusRCodeRnd (median=\CorpusRCodeMedian) lines of R code and
\CorpusNativeCodeRnd (median=\CorpusNativeCodeMedian) lines of native code
(C/C++/Fortran). Figure~\ref{fig:corpus} shows these packages, the size of the
dots reflects the project's size in lines of code including both R and native
code, the x-axis indicates the expression code coverage in percents and the
y-axis gives the number of reverse dependencies in log scale. Dotted lines
indicate means. Large (over \CorpusLargePackageTresholdRnd lines of code) and popular (in \CorpusMostDownloadedTreshold most downloaded) packages are annotated.
...
checked asserting certain quality.

Next to code and tests, R packages also contain examples and vignettes. Examples
are runnable snippets extracted from documentation. Vignettes are tutorial-like
guides into the package functionality. They are written in the literate
programming style using either Latex or Rmarkdown format with embedded code
snippets. Since CRAN is a curated repository, upon submission, it checks all the
runnable code in tests, examples and vignettes and only a successfully running
package is admitted to the archive. We use the same mechanism to extract
runnable code and run it for the dynamic analysis.

In the selected packages, there are \CorpusRunnableCodeRnd lines of runnable
code in examples (\CorpusExamplesCodeRnd), tests (\CorpusTestsCodeRnd) and
vignettes (\CorpusVignettesCodeRnd). Running this code results on average in
\CorpusMeanExprCoverage package code coverage. The code coverage was computed
for each package using

\begin{figure}[!tb]\centering\includegraphics[width=.9\linewidth]
  {programs-code.pdf}\caption{Programs code size}\label{fig:programs-code}
\end{figure}
\begin{figure}[!tb]\centering\includegraphics[width=.9\linewidth]
  {programs-runtime.pdf}\caption{Programs execution time}\label{fig:programs-runtime}
\end{figure}

\subsection{Kaggle sripts}
% contrast to CRAN - duplication
...
\eval~\cite{ecoop11}, the case for R is more striking at \miss{100\%} of the
...
programs in our corpus. The mean number of calls per run was \miss{20} (with a
min of \miss{1} and a max of \miss{10,000}). Table~\ref{A} details the usage of
...
can observe \miss{???}. {\bf <The following is a place holder. Real text will
  ...
  \resizebox{\columnwidth}{!}{%
    \begin{tabular}{l|rr|r||rr|r}
      ...
      \cline{1-1} \cline{2-4} \cline{5-7}
                                  & Core & Packages & Total & Core & Packages & Total\\
      \hline
      ...
      \c{local} & \CoreLocalCallCountRnd & \PackageLocalCallCountRnd  & \AllLocalCallCountRnd & \CoreLocalSiteCountRnd & \PackageLocalSiteCountRnd & \AllLocalSiteCountRnd \\
      \hline
      Total & \CoreAllCallCountRnd & \PackageAllCallCountRnd  & \AllAllCallCountRnd & \CoreAllSiteCountRnd & \PackageAllSiteCountRnd & \AllAllSiteCountRnd \\
      \hline
    \end{tabular}
  }
...
\begin{table}[ht]
  \resizebox{\columnwidth}{!}{%
    \begin{tabular}{rr|rr|rr}
      \hline
      Callsites & Packages & Callsites & Packages & Callsites & Packages \\
      \hline
      ...
      \SiteSummarySiteCountE & \SiteSummaryPackageCountE & \SiteSummarySiteCountJ & \SiteSummaryPackageCountJ & \SiteSummarySiteCountO & \SiteSummaryPackageCountO \\ 
      \hline
    \end{tabular}
  }
...
\miss{This is on a random sample of 10000 unique rows of the summarized data set as ST size takes a lof of time to compute. To update when it finishes computing...}
