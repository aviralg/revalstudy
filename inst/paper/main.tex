...
most frequent ten expressions to eval calls from core R contribute to 85\% of
all \eval calls.
\begin{table*}[tb]
  \centering
  \begin{tabular}{l|rr} \hline
    Expression & Eval Call & Eval Call \% \\\hline
    \c{c("auto", "shell", "radix")} & 1987105 & 29\%\\
    \c{c("auto", "shell", "quick", "radix")} & 1593169  & 23\%\\
    \c{\{info <- loadingNamespaceInfo(...} & 1008632 &       14\%\\
...
\c{force(..1)} on them.  The \c{force} function forces
promises~\cite{oopsla19a} and returns the result of evaluation. The
\c{inner} pattern arises from \c{glue::identity_transformer} which enables
the creation of custom transformation functions for affecting the
