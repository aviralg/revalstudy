...
properties of R programs --- but we can also monitor the inner working
of the R interpreter --- this allows us to capture details of
execution that are not exposed at the source level. Dynamic analysis
is of course limited in that it can only observe behaviors that are
triggered by the particular inputs passed to the program. Luckily,
CRAN libraries come with multiple input datasets and use-cases.
...
The expressive power of \eval depends on choices made by the language
designers along the following two axes:
\begin{itemize}
\item {\bf Scoping:} What environment does the string passed to \eval
  execute in? JavaScript and R evaluate it in the current environment,
  thus exposing local variables and parameters to \eval and breaking
  the abstraction boundary of the enclosing function. Julia is more
  restrictive, it runs \eval at the ``top level'', in the global
  environment. JavaScript has a global \eval that behaves like Julia,
  it also has a ``strict mode'' in which \eval is allowed to access
  local variables, but fobidden from injecting new variables in the
  local environment. In Java, one can implement \eval but the
  expression will execute in new environment.\footnote{While \eval
  used the purview of interpreted languages, the advent of
  just-in-time compilation lifted this restriction. To implement \eval
  in Java requires wrapping its input in a static method of a new
  anonymous class, generating bytecode for that class, invoking the
  class loader to install that code, and finally reflectively calling
  the new method. }
\item {\bf Reflective API:} What other reflective operations are
  ...
  (other than enumeration of the properties of an object and access to
  properties with strings), \eval is thus limited to what can be
  expressed in JavaScript code. R, on the other hand, has a rich
  reflective interface. For example, R allows user code to walk the
  call stack and arbitrarily add and delete local variables in any
  environment. This reflective API means that there are few limits to
  what can do.
\end{itemize}
...
The reason Julia restricts \eval to the top level environment is to
preserve the compiler's ability to optimize
functions~\cite{oopsla18a}. What the Julia designers wanted was to
prevent \eval from modifying the type of local variables as this would
require recompilation of the method. Julia even adds a versoning
mechanism, called world age, to ensure that code added during an \eval
does not invalidate optimizations such as inlining~\cite{oopsla20a}.
More permissive languages such as R are much harder to deal with from
a compiler's perspective.

\medskip\noindent{\it Eval in R.} There are four reflective functions
that take abstract syntax trees (called expressions), parse them as
code, and evaluate the result. These functions are \c{eval},
\c{evalq}, \c{eval.parent}, and \c{local}. Taking an expression rather
than a string, may discourage some casual users as manipulating
expressions is slightly more involved.\footnote{The rationale for
\eval taking expressions is explained by the main use case for \eval,
namely extending the language by reinterpretation of promises; R gives
access to the code of a promise as an expression.} There are two ways
to obtain an expression. Given a string, the \c{parse()} function
retuns an unevaluated expression. The other way is to use the function
...
\medskip\noindent\textit{Environments.} In R, environments can be
manipulated at will. Each function has an environment that can be
accessed with \c{environment()}. A new environment, which by default
inherits from the current environment, is created with \c{new.env}.
Contrary to other values, environments have reference semantics rather
than copy semantics. The chain of inheriting environments can be
traversed with \c{parent.env}, ultimately one arrives at \c{emptyenv}.
...
create new bindings in any environments. Given an environment \c{e}:

\begin{itemize}
\item  \c{e$v} or \c{get("v", envir = e)} reads variable \c{v} in \c{e};
\item \c{e$v <- 2} or \c{assign("v", 2, envir = e)} writes value 2 in variable \c{v} in \c{e}. \c{v} is created if it did not exist in \c{e}.
\end{itemize}
\noindent One can directly modify a variable in the scope of its
calling function, and there is even a special assignment operator for
that purpose; \c{v <<- 2} will search for \c{v} in parent
environments, redefine the value, or create a definition in the
current environment. Environments are used as hashtables, to maintain
state across function calls, or to create package namespaces.
