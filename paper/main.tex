...
\vspace{-2mm}
\includegraphics[width=7.5cm, trim=7.5cm 0 6cm 0, clip]{calls_per_run_per_call_site}
\vspace{-7mm}
...
\eval. Most of the invocations involving a symbol come from ggplot2, \Ggplotsymbolpercent, and from one call site, in \c{ggproto}, evaluating symbol \c{`_inherit`}.
...
AST nodes. For instance, a variable name (a symbol) counts for 1 node. Our de-parser does not emit code for values as some values
can be very large and would overwhelm the code. On average, arguments
to \eval are \Avgszeval nodes large, the median size is
\Medianszeval. The largest input observed is \Maxszeval.
Figure~\ref{fig:sizedistribution} shows the distribution of sizes. The
x-axis is the size of arguments in number of nodes, and the y-axis is the
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=.55\textwidth]{events_per_pack_small}
	\hspace{-10mm}
	\includegraphics[width=.55\textwidth]{events_per_pack_large}
	\caption{Events per packages} \label{fig:eventsPerPackage}
\end{figure}

\subsection{Things todo}

\begin{itemize}
\item check that on figure 7, the number of elements at 1 is correct
\item explain the peak at 4, for figure 7
\item Figure 8 explain why so few 1's
\item Add a ``Discussion'' paragraph at the end that sums up the
  observations
\item Add paragraphs for Kaggle and CORE -- only discuss the
  differences from CRAN if any.
\item Question: should we remove ggplot? Would change things
  significantly?
\end{itemize}
