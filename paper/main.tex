...
Analysis of the arguments passed to \eval reveals that XXX are of kind
\c{symbol} (single variables), XXX are \c{language} (an internal R
type roughly corresponding to a single expression), and XXX are
\c{expression} (one or more statements). The remaining arguments are R
values which are returned unchanged.
