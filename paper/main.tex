...
For instance, \code{c("Ha","bye")} evaluates to a vector of two strings. Constants
such as \code{42} are vectors of length one. To enable equational reasoning, values
...
attribute \code{dim}-\code{c(2,2)} can be attached to the value at \code{x} by
\code{attr(x,"dim") <- c(2,2)}. Adding this attribute turns \code{x} into a
matrix. The \code{class} attribute gives a value a 'class' in the object-oriented
sense. So, \code{class(x)<-"human"} sets the class of \code{x} to \code{human}; classes
...
The \eval function in R takes three parameters: an \code{expression} to evaluate
(\code{e}), an environment where to evaluate (\code{envir}), and an enclosure
(\code{encl}) that is used to look up objects not found in \code{envir}.\footnote{Next
to the \eval function R offers three other variants:
  \begin{inparaenum}[1.]
    \item \code{evalq} automatically quotes passed expression---a shorthand to
      \code{eval(quote(...))},
    \item \code{eval.parent(e, n)} specifies the evaluating environment in terms of
      number of call frames (\code{n}) to go back---a shorthand to \code{eval(e,
        parent.frame(n))}, and
    \item \code{local} evaluates \code{e} in a fresh environment---a shorthand to
      \code{evalq(e, new.env())}.
  \end{inparaenum} }
...
\begin{lstlisting}[caption={Examples of calls producing expression \code{a+b}}]
  parse(text="a+b")
  quote(a+b)
...
\end{figure}
 ...
 The call to \code{substitute(x)} extracts the unevaluated expression from
the promise \code{x}. The function \code{match.call} returns an expression
...
integer \code{n} (in which case it would use the \code{n.} call frame). If no
environment is specified, then the default is the environment where the call to
\eval was made (as returned by \code{parent.frame}).
%
Environments nest, each has a parent. When creating a new environment with
\code{new.env}, the parent is the current environment. Environment chains can be
traversed with \code{parent.env}, until \code{emptyenv} is reached. The top-level
environment is \code{.GlobalEnv}, it has parents that represent the packages that
...
than 2\% of methods. The most common reflective method is \code{perform:}; it send
...
of those. The \code{match.arg} function is the highest used one with 54\% of all
calls. In the other call sites, they saw two use cases. The most common is the
evaluation of the source code of a promise retrieved by \code{substitute} in a new
environment, \eg as done in the \code{with} function. The other use case is the
invocation of a function whose name or arguments are determined dynamically. For
this purpose, R provides \code{do.call} and thus \eval is overkill. Our corpus is
...
\item \emph{Extract.} The \genthat tool extracts all runnable code snippets and
  turns each of these into a self-standing program~\cite{issta18}, \code{knitr}
...
\item \emph{Analyze.} Analysis outputs are merged, cleaned, and summarized.
  ...
  from \code{ggplot2}) and numbers (exported as \LaTeX macros) appearing in the
  paper.
\end{compactenum}
...
arguments (it taints the results of calls to \code{parse} and \code{match.call}). It
...
\begin{tabular}{@{\,}r|rrrr}\toprule
  &\eval & \code{evalq} & \code{eval} & \code{local}\\[-1.5mm]
           & & & \code{.parent} &\\\midrule
...
calls go to \eval itself, \code{eval.parent} is rare, and both \code{evalq} and
\code{local} are barely used at all.
...
arguments. More specifically, \packageSymbolpercent are \code{symbol}s (variables
such as \code{x}), \packageLanguagepercent are \code{language} objects (expressions
representing a function calls), and \packageExpressionpercent are \code{expression}
objects (lists of expressions). Further inspection reveals that \code{\_inherit}
accounts for \packageGgplotsymbolpercent of symbols and comes from one site in
\code{ggplot2}.\footnote{Each object has a field model inheritance, this comes from
\code{ggproto}, one of the many object-oriented systems for R.}
...
\code{expression(x+1)} has a size of 3. The large number of symbols skew the median
...
forms use \code{V} to stand in for values occurring in expression, \code{X} stands in
for variables, and \code{F} stands for functions. The function performs constant
...
\packageNbSymbolVarSitePercent are simple variable names, e.g. \code{x}. The shapes
subsumes \code{V} and built-in arithmetic operations, so a mixture of arithmetics
and variables normalize to \code{X}. A lookup is one step of execution, if a promise
...
\\
\medskip\EE{$min(e)=\c{\$}$}\\[-2mm]\small This shape extends \code{X} to include lookup
with the dollar operator, \eg~\code{x\$f}, and vector indexing, \eg~\code{x[42]} and
\code{x[[24]]}. This takes a few more step of evaluation on average, and is
typically used in a function environment.
\\
\medskip\EE{$min(e)=$~\code{<-}}\\[-2mm]\small This includes both direct assignment {\tt
  <-} and parent environment assignment {\tt <\,\!<-} as well as the \code{assign}
function. It subsumes \code{\$}. Assignments represent the most obvious source
...
\begin{tabular}{@{}p{.97\linewidth}}
  ...
  computation, \eg~\code{function(x) x+1}. In addition to \code{FUN},
...
frameworks (\eg \code{testthat}, \code{testit}), code benchmarking (\eg
\code{rbenchmark}, \code{microbenchmark}), running code in parallel (\eg \code{foreach},
\code{doParallel}), or deferring code execution (\eg \code{withr}).


\\
\medskip\EE{$min(e)=\c{model.frame}$}\\[-2mm]\small The \code{model.frame} function
returns a dataframe resulting from fitting the model described in a formula.
This shape subsumes \code{F(F(X))}, \code{FUN}, and assignments. It is the single most
...
\code{magrittr} package which is used to compose functions, \eg instead of
...
\code{env} argument. This gives users control over what is visible to the
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		\node (E) at (2,0) {\code{eval(e)}};
		\node[text width = 3cm] (CA) at (2, 1.5) {\code{e[[2]] <-quote(c)}};
		\node (Q) at (4, 3) {\code{quote(c)}};
		\node[text width = 4cm] (A) at (0, 3) {\code{e <- parse(text = "a;b")}};
		\node[text width = 3cm] (P) at (0, 4.5) {\code{parse(text = "a;b")}};
...
The provenance of \code{e} includes both \code{parse} and \code{quote}. We pick an origin
...
to yield the expression passed to \eval. In our example, the origin is \code{parse}.
...
\begin{itemize}[---]
\item {\it Reflection:} use of \code{match.call} to reflectively capture the
  expression that invoked this function, arguments are promises, and
  \code{substitute} is used to retrieve their source.
\item {\it String:} create from strings by invoking \code{parse},
  \code{str2expression}, or \code{str2lang}.
\item {\it Constructed:} invoked \code{quote} or \code{expression}, or by built with
  \code{call} or \code{as.call}.
\item {\it Environment: } created with \code{as.name} or \code{as.symbol}, typically
...
dominate the side-effect data.\footnote{The corpus has \code{RUnit, testthat,
  tinytest} and \code{unitizer} testing frameworks.}
...
\code{.Random.seed} variable is saved and restored from and to the global
...
functions: \code{plyr::allocate\_column} (allocates space for a new data frame
column), \code{withr::execute\_handlers} (executes deferred expressions), and
\code{foreach::doSEQ} (executes an expression on each element in a collection,
possibly in parallel). Most of the sites (\SESitesInEnvirRatio) update
the environment specified by the \code{env} parameter.
...
(\code{withr::execute\_handlers}). This makes sense as it is used to defer
evaluation of an expression to after the function exit and thus used for clean
up. It is used almost exclusively by the \code{tidyselect} package for removing the
...
manual injection of parameters into a \code{model.frame} execution environment, we
...
Out of the minimized expressions, it is \code{BLOCK}, \code{<-} and \code{F(F(X))} that
contribute to the vast majority of side effects. Concretely, 92\% of \code{BLOCK},
87\% of \code{<-} and 15\% of \code{F(F(X))} expressions do side effects. In the case
of \code{BLOCK}, most of them happen in the environment passed to the \eval call
(default is the parent environment). For \code{<-} it is 15\%. However, this form
is dominated by the \code{plyr::allocate\_column} function that side-effects in
the environment containing the data frame to which it allocates new column.
Without this call, \code{<-} is similarly predictable with 96\% of side effects
happening in the passed environment. For \code{F(F(X))}, it is less clear as 58\%
...
evaluation, \code{substitute} and \code{eval} are some of the tools given to
...
the text can refer to some variable \code{x} that will be set up by the called
...
construct function objects that are parameterized with string \code{what}.
...
an expression that represents a call to \k{alist} but where the \code{...} are
expended with the arguments supplied to the function. \Eval then returns a
\code{list} with unevaluated argument expressions which are evaluated
...
\code{eval(substitute(alist(...)))}. The expression are evaluated sequentially
...
\paragraph{Shadowing}
%%% TODO ?????????????////
Many packages use \eval to implement variations of the \code{base} R functions.
For example, the \code{igraph} package provides the \code{do_call} function
shown below. This is a variant of the \code{do.call} function from the
\code{base} package. This function executes a function call by constructs it
...
via \eval and \code{match.call}. The example uses \code{match.call} to access
the expression with which the function is called. The next three lines construct
a call to \code{stats::model.frame} with argument text of parameters
\code{'formula'}, \code{'data'}, and \code{'weights'}. Then, the call expression
is evaluated in the parent environment. The use of \code{match.call}, avoids
repeated use of \code{substitute} for extracting the argument expressions. This
...
\code{"formula"} attribute attached to the \code{pdMat} object and evaluates it.
...
Some packages use \eval to bypass the restrictions imposed by the \code{R CMD
...
use \eval to obfuscate their code. The example mutates the variable \code{s} in
the package environment. However, that is locked by default and
\code{unlockBinding}, is a restricted function. To work around this, the call to
...
\emph{Variable lookup and values, and indexing \code{(X, V, \$)}}
    constitute one of the most common use case for \eval. Let's consider the
    \code{subset} function from the R base library, which allows, among others, to
    select variables in a data frame. For example, given a data frame \code{df}
    with three columns \code{A,B,C}, \code{subset(df, sel=A)} returns a subset of
    \code{df} with just the \code{A} column. The same would happen with \code{subset(df,
      sel=1)}. However, thanks to \eval it can also be also called with
    \code{subset(df, sel=c(1,B,x))} returning a subset of the first column, column
    \code{B} and whatever index the variable \code{x} holds. It is implemented using:
    \lstinline|eval(substitute(sel), nl, parent.frame())| The first argument
    returns the expression passed to \code{sel}, the second is the data frame in
    which this expression will be evaluated, and the last indicates which
    environment should be used to look up bindings not found in \code{nl}.


 \emph{Assignments \code{(<-)}} appear in the cases where a developer wants
    ...
    \code{parse} or \code{substitute}, often in a loop. For example, the
    \code{PopED::get\_rse} uses it to assign a number of variables in the current
    scope: \lstinline|eval(parse(text=paste(default_args[[i]]), "<-", i))|.


 \emph{Function calls \code{(F(), F(X), F(F(X)))}} represent two very
    ...
    ad-hoc calls to libraries such as \code{ggplot2} when one needs to
    ...
    \code{A} and \code{B} columns in \code{df}. It might not be immediately
    clear\footnote{\cf
    ...
    judicious use of \eval. For example, the \code{data.table} package uses \eval
    to call the \code{options} function with named arguments taken from a vector
    of strings:
...
such as \code{X}, \code{V}, \code{\$}, \code{<-}, and \code{FUN}. Variable lookup in a remote
environment can be performed by the builtin \code{get} function, a similar case
can be made for assignment and the \code{assign} function. There is a dedicated
\code{do.call} function to build function calls. Unnecessary uses of \eval
...
function body uses \code{get} to perform the lookup and then calls directly.
...
Another example, from \code{coin::copyslots}, constructs an expression that access
...
As a last example, \code{configr::config.funs.par} construct three expressions
that are then fed to \eval. Semantically equivalent code would use \code{get}
to lookup \code{fun}, and then directly execute the three expressions.
...
\code{rlang::eval\_tidy}~\cite{tidyverse}. This departs from \eval in two ways.
First, it supports evaluation of quosures, custom objects used by for
metaprogramming that bundle expressions with an environment. \code{eval\_tidy}
...
in the data mask. Expressions such as \code{return()} do not work since the data
