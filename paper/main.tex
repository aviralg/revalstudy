...
corpus. In this section we show 9 real-world examples from this batch. The
goal is to illustrate the variety of use cases that employ \eval in the R
ecosystem. The code snippets are simplified for clarity.
...
as the extracting varags example and evaluates each of the expression
...
statistical modeling, \ie the \c{model.frame} shape in Table~\ref{tab:minimizedexpressions}.
...
call's argument to another function is also used to created wrappers. For
...
harder than in other languages because of the default argument values, the
varargs and the missing arguments.

% % TODO: update to explain why the eval is needed or remove
...
The \k{glue} package contains a variant that leverages the non-standard scoping
allowing to resolve interpolation in an alternative environment. For example,
the following snippet pipes the subsetted data frame into the \k{glue_data}
function that uses it to resolve the symbols in string interpolation.
...
\k{do.call} function to perform function calls. However, the unnecessary uses
of \eval are not easy to find and often can only be uncovered by manual
analysis. Dynamic analysis is limited due to coverage issues, and static
analysis of R remains an open problem.
...
For example in Listing~\ref{lst:unnecessary}, the function uses \eval to lookup
a target function name from a constructed string. Then it constructs a calls it
with the arguments that are passed to the current function. \Eval is overkill
in this case: a semantically equivalent function body can use \k{get} to perform
the lookup and then call the resulting function directly.
...
fields of an object and copy them. 
...
This can be replaced by symbolic field reads
and writes:
...
% TODO: remove - does not bring anything new or interesting
% As a last example, \k{configr::config.funs.par} constructs three expressions
...
used by for metaprogramming that bundle expressions with an environment.
