...
The corpus is assembled from three sources: the \emph{Base} libraries bundled
with the language implementation, packages hosted on \emph{CRAN}, and scripts
from \emph{Kaggle}. Each of these sources represents a different part of
the user base of the language and thus, potentially, different usage patterns.

\mypara{Base.} The \BasePackages libraries are bundled with R and widely used as
they provide arithmetics, statistics, and operating system functionalities. Some
\BaseFunsWithEvals functions contain \BaseEvalCallSites calls to \eval. The
functions perform such key tasks such as package loading, there is thus hardly
any R program that does not invoke them. These libraries have been written by
experts with a deep understanding of the language and its environment.

\mypara{CRAN.} This dataset consists of packages hosted on the Comprehensive R
Archive Network ({\small \url{cran.r-project.org}}, aka CRAN). There are three
sources of runnable code in a package: unit \emph{tests} for individual
functions, code \emph{examples} automatically extracted from the documentation,
and long-form use-cases called \emph{vignettes}. Package also often have sample
input files. All of these runnable code snippets can be extract into independent
files. We downloaded \CranPackages packages and extracted \CranRunnableScripts
scripts, or \CranRunnableCode lines of code (\CranRunnableCodeExamplesRnd in
examples, \CranRunnableCodeVignettesRnd in vignettes and
\CranRunnableCodeTestsRnd in tests). CRAN packages are authored by experienced
developers.

\mypara{Kaggle.} This dataset contains \KaggleUnique programs from the Kaggle
website ({\small \url{kaggle.com}}), an online platform that allows users to
submit and compete to solve problems. Kaggle contains both scripts and notebooks
as well as input files, but there is some duplication (we used SHA-1 hashes to
remove \KaggleDuplicates duplicates). The dataset has 665K lines of R code and
only \KaggleWithEvals calls \eval. Kaggle scripts are written by end-users with
...
\noindent The pipeline runs in parallel on three servers, each with 2.3GHz Intel
Xeon 6140 processor with 72 cores and 256GB of RAM, and shared OCFS network
storage. Servers run identical docker images with all dependencies installed.
