...
\item Check that the percentage of operations performed within
  an \eval (5\%) is correct. That feels surprising to me. We should actually remove this number!
\end{itemize}
...
However, we do not want to discriminate expressions based on specific values. Hence, the expression goes through several simplification steps: constant folding of arithmetic and string expressions that use commonly used functions (addition, string concatenation), discarding of specific value to a symbolic value holder, variable absorption. We keep track of variables, as they are evaluated in the environment specified to \eval, contrary to values that are left unchanged.  After the simplification steps, we obtain 15 different \emph{minimized} expressions.
...
\begin{table}[h]
	\begin{tabular}{|c|c|c|c|c|c|}
		...
		\hline
		\texttt{\{BLOCK\}}& \packageMinimizedcallsitesi  & \packageMinimizedpropsitesi & \packageMinimizedpackagei & \packageMinimizedoperationsiRnd & \packageMinimizedpercentenviri \\
		\hline
		\texttt{FUN} & \packageMinimizedcallsitesj & \packageMinimizedpropsitesj & \packageMinimizedpackagej & \packageMinimizedoperationsjRnd & \packageMinimizedpercentenvirj \\
		\hline
		&  &  &  &  &  \\
		\hline
		&  &  &  &  &  \\
