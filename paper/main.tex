...
\eval usage in the wild. All of the examples are based on code from our corpus
...
% We start the discussion with the examples for the minimized expressions from
% Section~\ref{sec:minimized}.
...
\begin{compactitem}[---] 
  ...
  \item \emph{Function calls \c{(F(), F(X), F(F(X)))}} represents two very
    frequent patterns: \emph{code generation} and \emph{call reflection}. There are numerous
    cases for code generation. One occurring usage is generating ad-hoc calls
    ...
    how modify the call so it will pick the actual mapping from \c{A} and \c{B}
    ...
    \eval to calls the \c{options} function with named arguments taken from a
    ...
    Call reflection refers to the combination of \eval with \c{match.call},
    function that returns the current call with all its arguments captures as
    expressions. This is used mostly in the following circumstances: (1) to
    record a call for a later use in possibly different environment, (2) pass
...
shines when expressions passed to it can be of many different types. % And it
