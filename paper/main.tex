...
looked at the use of \eval in their corpus (1238 CRAN packages, R base
libraries, and 515 Bioconductor packages). They found 8500 \eval call sites and
recorded 2.2M \eval calls at runtime. Because they included base, the results
were dominated by a few functions (15 call sites accounted for 88\% of \eval
calls) with \c{match.arg}\footnote{A popular base function that matches an
  argument against a set of candidate values that uses \eval to get the
candidate values.} being responsible for over half of the recorded \eval
calls. They discussed two use cases. The first one is the evaluation of an
expression extracted from a promise using the \k{substitute} call in a new
environment. The other is the invocation of a function with dynamically
computed names and arguments.
...
Next, we track the number of information such as the shapes of expressions
