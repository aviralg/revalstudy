...
the \code{see_\if} function shown below. It accepts the conditions as arguments
...
to \code{...} (vararg). For a non-vararg parameter \code{x}, the expressions
\code{substitute(x)} can be used to extract the argument text. However, for \code{...}
arguments, the expression \code{eval(quote(substitute(list(...))))} or its variants
are used. The \eval call evaluates the quoted expression
\code{substitute(list(...))}. This returns a \code{list} of unevaluated argument
expressions. The \code{hasGroupNumber} function from the \code{lattice} package is one
of the many users of this pattern.

The \code{withr} package provides the \code{defer} function which defers the
