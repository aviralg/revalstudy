...
site has a single kind. It means that we can predict the \emph{shape} of the expression passed to \eval in \packageNbOneCategoryEnvirSitePercent of the sites.
...
come global and package. Wrapping global is more widespread than directly accessing global.
...
\SEAllCalls \eval calls in \SEAllSites sites. Again, the challenge is to remove
the accidental side-effects caused by the R virtual machine implementation and
are not related to the user code. For example, the \c{.Random.seed} variable,
which contains the state of the random number generator is saved and restored
from and to the global environment every time the user calls one of the base
routines that use random numbers. Removing them leaves us with \SEUser side
effects (\SEUserRatio) from \SEUserCalls \eval calls (\SEUserCallsRatio) in
\SEUserSites sites (\SEUserSitesRatio). These sites are part of
\SEUserFunctions in \SEUserPackages packages. \SEFunsNighty functions are
responsible for 90\% of side effects. Half of the side-effects come just from
three functions: \c{plyr::allocate\_column} (allocates space for a new data
frame column), \c{withr::execute\_handlers} (executes deferred expressions), and
...
to have a given target environment class, all of its \eval sites that do side
effects must have the same target environment class The majority of the \eval
sites (\SESitesInOneClass) do all side effects consistently in one environment
class. The same happens on the function level. Almost half of the sites (over a
third of the functions) do side effects in either \emph{Local} or \emph{Object}
environments. This gives a ray of hope for the compiler. Even though it is
possible to do anything anywhere, the data suggest that most side-effects are
sane.
...
The examples are based on code from CRAN or Kaggle, simplified to fit the limited space of a scientific paper. We indicate the package and the function where the example is located.

\subsection{Shapes of \eval}

We group minimized expressions in larger categories.
...
It is the predominant usage of \eval in Base, through function \c{match.args} (with \eval on minimized expression \c{V}). In CRAN, it correlates with more exotic environments passed to \eval. 
...
Package \emph{statnet.common} handles formulas, R object that associates an expression of the form \c{x~\textasciitilde~expr} and a carry-on environment where to evaluate the formula. It evaluates the left-hand side of the formula, which is always a variable, in function \c{eval\_lhs.formula}, as \c{eval(object[[2L]],envir=environment(object))} where \c{object} is the formula and \c{environment} returns the formula associated to the object.
...
We present here high-level patterns that we identified after manual inspections of hundred of \eval sites. Some of the patterns were also described in the R documentation.
...
In this paper we provide large-scale study of the use of the \eval function in
the R programming language. It is based primarily on a corpus of
\CranRunnableScripts scripts extracted from \CranPackages R packages. As
expected, \eval is used mainly in libraries and therefore we have found just a
few \eval call sites in scripts coming from Kaggle that we included to
represent code written by R practitioners. We used an instrumented version of
the R virtual machine, \rdyntrace, to capture all \eval calls traces
including the relevant side effects that happen within these calls. There is a
number of insights that can be learnt from the data.
%
\eval is widely used in the R core packages that come bundled with the
...
Putting the core packages aside, in CRAN, \eval is used by \PkgPackagesRatio
of the available \CranPackages packages.
%
The code executed by R \eval has unusual expressive power. It can essentially
modify anything anywhere which is wary for both R developers as well as R
compiler.
% TODO expressions passed to eval
% TODO side effects
