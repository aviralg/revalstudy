...
to \eval and is not \c{parent.frame()}.
...
\c{BLOCK} & \packageMinimizedcallsitesj & 4\% & \packageMinimizedpackagej & 440K & 73\% & \c{\{ write.csv(iris, tf) \textbackslash n file.size(tf) \}} \\\hline
...
15 normal forms. We detail the interesting cases next.
...
value.\footnote{True as long as base functions such as \c{+} are not redefined.
They typically aren't, but this is a limitation of this categorization.} In our
corpus, \packageNbCallSitesUniqueActualValue call sites only ever see a simple
values; the median number of times they are exercised is
\packageMedianRunSitesUniqueActualValue, suggesting a possible lack of coverage.
However, manual inspection reveals cases such as:
\begin{lstlisting}
...
\medskip\noindent\EE{$min(e)=\c{X}$} Variables are the most common form. They
...
just an \c{x}. As a variable may refer to a promise, it may trigger computation.

\medskip\noindent\EE{$min(e)=\c{\$}$} A name lookup, \c{x\$f}, can be used to
access variable in an environment or a column in a data frame, this also include
vector indexing operators, \c{[]} and \c{[[]]}. A name lookup subsumes
variables.

\medskip\noindent\EE{$min(e)=$~\c{<-}} Assignments account for
\packageAssignSitesPercent of \eval sites, they are \c{<-}, {\tt <<-} and
\c{assign}, and they subsume named lookups.

\medskip\noindent\EE{$min(e)=\c{F()}$} A plain function call which may have some
...
\medskip\noindent\EE{$min(e)=\c{F(X)}$} Function calls whose arguments may include
variable references.

\medskip\noindent\EE{$min(e)=\c{F(F(X))}$} Nested function calls with variables.

\medskip\noindent\EE{$min(e)=\c{FUN}$} Function definition accounts for
...
\medskip\noindent\EE{$min(e)=\c{\{\,\}}$} Arbitrary-length statement blocks.
These are larger expressions with, on average, \packageMinimizedoperationsjRnd
instructions. The contents of the block are not inspected.

\medskip\noindent\EE{$min(e)=\c{model.frame}$} Call to the \c{model.frame}
function used to create statistical models. This subsume all other categories.
