...
An example is the \code{assertthat} package's \code{see\_if} function which is used to
assert that the sequence of conditions supplied as arguments are true. It
evaluates the condition expressions one-by-one inside a \code{tryCatch} block and
produces an error at the first expression that returns \code{FALSE}. If the
expression itself produces an error, the error message is captured by the
enclosing \code{tryCatch} block and returned. The remaining expressions in the
sequence remain unevaluated. A similar pattern is used by the \code{checkmate}
package.
