...
\begin{tabular}{@{}p{.97\linewidth}}
  \medskip\EE{$min(e)=$ \code{V}} \\[-2mm]\small Values occur in 16\% of sites, 74\% of
...
\end{lstlisting}
\\
\medskip\EE{$min(e)=$ \code{X}}\\[-2mm]\small Variable lookups are the most common shape,
...
\\
\medskip\EE{$min(e)=$ \code{$}}\\[-2mm]\small This shape extends \c X to include lookup
...
\\
\medskip\EE{$min(e)=$ \code{F()}}\\[-2mm]\small Function calls without variable lookups or
...
\\
\medskip\framebox{$min(e)=$ \code{F(X)}}~\EE{$min(e)=\c{F(F(X))}$}\\[-2mm]\small Function
...
\\
\medskip\EE{$min(e)=$ \code{BLOCK}}\\[-2mm]\small Multi-statement code blocks, as
...
environments.
\\
\medskip\EE{$min(e)=$ \code{model.frame}}\\[-2mm]\small The \c{model.frame} function
