...
mostly irrelevant (unless it redefines a built-in operator).
%% Why then this variety of environments??
...
package or its \emph{namespace}. A call to
...
%TODO: there is a difference between a package environment and a namespace. Shortly explain.
...
We classify environments to \eval as follows: 

\begin{itemize}
	\item function environments: the frame of the calling function or one of the parent frames (obtained using \c{parent.frame()}, \c{sys.frame()} or with an integer argument for \c{envir} for instance);
	\item list environments: a list, a data frame, a tibble, or \c{NULL} (seen as the empty list), passed to \eval;
	\item the global environment;
	\item package and namespace environments;
	\item the empty environment (created with \c{emptyenv()})
    \item new environments, created with \c{new.env}, \c{list2env} or \c{as.environment}.
\end{itemize}


As shown in Table~\ref{tab:highlevelenvironments}, most calls happen in a function environment, with \c{global} as a distant second. 
...
\begin{table}[h]
	...
	\begin{tabular}{|c|c|c|}
		...
		\hline
		Function environments &  \packageNbFunctionEnvSites &  \packageNbFunctionEnvSitePercent\\
		Global &  \packageNbGlobalEnvSites & \packageNbGlobalEnvSitePercent \\
		List &  \packageNbEnvListSites & \packageNbEnvListSitePercent \\
		Package & \packageNbPackageNamespaceEnvSites & \packageNbPackageNamespaceEnvSitePercent \\
...
Table~\ref{tab:framenumber} shows the function environments classified according to their position in the call stack: $0$ is the current function, $1$ the parent function and so on.

The calls happening in a function environment are mostly happening in the environment of the function calling \eval. This corresponds mostly to the default \c{envir} argument of \eval; indeed, \packageDefautEnvirSitesPercent of the sites have that default argument. A lot of sites also use \c{parent.frame()} to access their parent frame. It corresponds to environment class \c{1} but also to \c{global} when the function is called at the top level. Explicitly and directly referring to the global environment using \c{globalenv()} or \c{.GlobalEnv} is rare, as it happens only in \packageNbExplicitGlobalSites sites. Accessing further-away frames happens much more seldom.

\begin{table}[h]
    \centering
    \begin{tabular}{|c|c|c|}
        \hline
        Frame number & \#sites & \%sites \\
        \hline
        ...
        > 3& \packageNbFarAwayCallerSites &  \packageNbFarAwayCallerSitePercent \\
        \hline
    \end{tabular} \label{tab:framenumber}
\caption{Frame number for the function environments. One site can possibly be passed different kinds of environments.}
\end{table}

A new environment creation, a parent environment must be specified. In Table~\ref{tab:newenvs}, we show what environments the new environment inherits from ultimately.\footnote{It means we directly show the first parent that is different from a new environment.} New environments mostly inherits from function environment: that is actually the default for \c{new.env}, with argument \c{parent = parent.frame()}. A large number of new environments are also wrapping package environment or namespaces. This is a widespread pattern to store variables to use at the package level. When a new environment wraps the empty environment, it can be handily used as a hash table.\footnote{\c{new.env} builds an environment that stores the variable name with a hash table by default. }
% Speak also about new and global?
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        \hline
        \packageNewEnvCategoryNamea & \packageNewEnvCategorySitesa & \packageNewEnvCategorySitePercenta \\
        \packageNewEnvCategoryNameb & \packageNewEnvCategorySitesb &  \packageNewEnvCategorySitePercentb\\
        \packageNewEnvCategoryNamec & \packageNewEnvCategorySitesc & \packageNewEnvCategorySitePercentc \\
        \packageNewEnvCategoryNamed & \packageNewEnvCategorySitesd & \packageNewEnvCategorySitePercentd \\
        \hline
    \end{tabular} \label{tab:newenvs}
\caption{Classification of environments wrapped by one or several \c{new}, in number of sites. Some sites can be passed more than one kind of environments. }
\end{table}
