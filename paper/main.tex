...
library code and the \Corpus packages in our CRAN corpus. For these
packages, all runnable code in CRAN was executed, runs that did not
exercise an \eval in the corpus were discarded. The data presented
here was collected from \Nbruns runs. In total, \Allcalls invocations
of \eval and its variants were recorded. Out of the
...
calls; and a similar number of packages, \Manycalls to be precise, use
\eval more than 1,000 times. One package, \Maxcallspack, calls it
\Maxcalls times and thus accounts for over half of the observed
invocations of \eval.
...
to \eval itself; \c{evalq} and \c{local} are barely used at all.
...
\begin{wrapfigure}{r}{5.5cm}
  \vspace{-8mm}
    \includegraphics[width=.4\textwidth]{calls_per_run_per_call_site}
    \caption{Normalized invocations
      (small)} \label{fig:callsitesnormalized}
   \vspace{-8mm}
\end{wrapfigure}

The majority of call sites, \Runbina, are exercised up to 50 times.
Larger numbers of calls suggest that \eval may be called within a loop
or from a recursive context -- this seems to be the exception.
Figure~\ref{fig:callsitesnormalized} zooms in on call sites with an
average of fewer than 51 invocation. The x-axis shows the number of
invocations and the y-axis is the number of call sites. Most call
sites are invoked once, about half as many are invoked twice. After
that the frequency quickly drops.

Analysis of the arguments passed to \eval reveals that \Symbolpercent are of kind
\c{symbol} (single variables), \Languagepercent are \c{language} (an internal R
type roughly corresponding to a single expression), and \Expressionpercent are
\c{expression} (one or more statements), for a total of \Codepercent of the \eval calls. The remaining arguments are R
values which are returned unchanged. \NOTE{closures are forced. (but there are nearly none in our dataset)}



\mypara{Amount of code loaded by eval.} We use \c{deparse} to turn the
resolved input to \eval into strings and measure the size of that input. The
largest input is \Maxszeval and the median input size is
\Medianszeval. Most inputs are small, less than \SzninefiveRnd
characters. The average size is \Avgszeval characters. 
Figure~\ref{fig:sizedistribution} shows the distribution of sizes.

\begin{figure}[h]
	%\centering 
	\includegraphics[width=\textwidth]{size_loaded_distribution}
	\caption{Distribution of the sized of the expressions loaded by \eval.} \label{fig:sizedistribution}
\end{figure}

\mypara{Amount of computation via eval.} We compare the number of
events intercepted during execution of \eval across the whole corpus.
For this, we use the number of interpreter steps as a proxy for
events. We observe \AllEventCountRnd \NOTE{Check that number.}
events all told, and \EventsRnd events occur inside eval
(\EvalEventAllPerc of all events). Figure~\ref{fig:eventsPerPackage}
show a violin plot of events.

\begin{figure}[h]
	\centering
	\includegraphics[width=.4\textwidth]{events_per_pack_small}
	\includegraphics[width=.4\textwidth]{events_per_pack_large}
...
is \MaxeventsRnd. However, a majority (\Smalleventspct) of \eval calls
performs up to 50 events suggesting that most expressions passed to
