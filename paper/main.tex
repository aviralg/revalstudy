...
as their arguments. We build a direct acyclic graph that track the provenance
of the values that are given to \eval. Fig~\ref{fig:provgraph} shows an example
of the provenance graph for the expression \k{e}.
The provenance of \k{e} includes both \k{parse} and \k{quote}. We pick an
origin by traversing the graph from the root, following the left-hand side
member of assignments until a leaf. It represents the provenance that is
...
\begin{minipage}{.59\textwidth}
	...
	\end{tikzpicture}
\end{minipage}
\caption{Example of a provenance graph} \label{fig:provgraph}
\end{figure}
