...
often in a loop. For example (taken from \c{PopED::get\_rse}), the following
code will populate the current scope with a number of variables:
%
\begin{lstlisting}
for (i in seq_along(args)) eval(parse(text=paste(args[i], "<-", i)))
\end{lstlisting}
...
\c{subset}, which selects columns in a data frame, uses it to allow to specify
the selected columns using either the column name or index. For example
\c{subset(df, sel=c(1,B,x))} returns a subset of \code{df} with the first
