...
string into expression) and \c{match.call} (reflects on current call). We
also capture calls to R API for dynamic code loading (\eg\xspace \c{library},
...
references for top-level calls in a function body or in the block surrounded by
braces. That is to say, a method whose body would \c{eval(x)} would not have
...
package in 3.2K C++ and 1.3K of R code. For performance reasons, most of the
...
to analyze function arguments while tracing (3) running large amount of
real-world code triggers exercises a lot of corner cases of highly
