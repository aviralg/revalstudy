...
\mypara{Simplify Interfaces} A common use of \eval is to simplify interfaces by
...
\mypara{Code Generation} The \c{Rcpp} package which bridges R with C++ is a
...
\mypara{Extracting varargs} A surprising use of \eval is the
...
\mypara{Conditional Evaluation} Developers frequently use \eval to explicitly
...
\mypara{Core function variants} Many packages use \eval to implement variations of the
...
\mypara{Domain-Specific Languages} The combination of laziness, metaprogramming,
...
\mypara{Boilerplate Removal} The combination of \code{match.call} and \code{eval}
...
\mypara{Code Obfuscation} Some packages use \eval to bypass the restrictions
...
\mypara{Miscellaneous}

\mypara{Lookup and indexing}

\mypara{Attributes}
Package \code{nlme} uses \eval to evaluate expressions attached as attributes to
its objects. Consider the definition of the \code{formula.pdMat} function below.
