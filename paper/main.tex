...
corpus. In this section we show 10 real-world examples from this batch. The
...
\paragraph{Extended Interfaces} Sometimes it is convenient to allow users to
...
this case users could provide an anonymous function.
...
as well allowing to resolve interpolation in an alternative environment. For
example, the following snippet pipes the subsetted data frame into the
\k{glue_data} function that uses it to resolve the symbols in string
interpolation.
...
\eval often can only be uncovered by manual analysis.
