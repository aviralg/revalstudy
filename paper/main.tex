...
The \eval function in R takes three parameters: an \k{expression} to evaluate
(\k{e}), an environment where to evaluate (\k{envir}), and an enclosure
(\k{encl}) that is used to look up objects not found in \k{envir}.\footnote{Next
to the \eval function R offers three other variants:
  \begin{inparaenum}[1.]
    \item \k{evalq} automatically quotes passed expression---a shorthand to
      \k{eval(quote(...))},
    \item \k{eval.parent(e, n)} specifies the evaluating environment in terms of
      number of call frames (\k{n}) to go back---a shorthand to \k{eval(e,
        parent.frame(n))}, and
    \item \k{local} evaluates \k{e} in a fresh environment---a shorthand to
      \k{evalq(e, new.env())}.
  \end{inparaenum} }
...
\begin{lstlisting}[caption={Examples of calls producing expression \k{a+b}}]
  parse(text="a+b")
  quote(a+b)
...
\end{figure}
 ...
 The call to \k{substitute(x)} extracts the unevaluated expression from
the promise \k{x}. The function \k{match.call} returns an expression
...
integer \k{n} (in which case it would use the \k{n.} call frame). If no
environment is specified, then the default is the environment where the call to
\eval was made (as returned by \k{parent.frame}).
%
Environments nest, each has a parent. When creating a new environment with
\k{new.env}, the parent is the current environment. Environment chains can be
traversed with \k{parent.env}, until \k{emptyenv} is reached. The top-level
environment is \k{.GlobalEnv}, it has parents that represent the packages that
...
of those. The \k{match.arg} function is the highest used one with 54\% of all
calls. In the other call sites, they saw two use cases. The most common is the
evaluation of the source code of a promise retrieved by \k{substitute} in a new
environment, \eg as done in the \k{with} function. The other use case is the
invocation of a function whose name or arguments are determined dynamically. For
this purpose, R provides \k{do.call} and thus \eval is overkill. Our corpus is
...
arguments (it taints the results of calls to \k{parse} and \k{match.call}). It
...
arguments. More specifically, \packageSymbolpercent are \k{symbol}s (variables
such as \k{x}), \packageLanguagepercent are \k{language} objects (expressions
representing a function calls), and \packageExpressionpercent are \k{expression}
...
graph with five nodes on Figure~\ref{fig:provgraph} for expression \k{e}.
...
The provenance of \k{e} includes both \k{parse} and \k{quote}. We pick an origin
...
to yield the expression passed to \eval. In our example, the origin is \k{parse}.
...
\item {\it String:} create from strings by invoking \k{parse},
  \k{str2expression}, or \k{str2lang}.
...
\k{invokeRestartInteractively} that prompts the user for input and \evals it.
...
\emph{Variable lookup and values, and indexing \k{(X, V, \$)}}
    constitute one of the most common use case for \eval. Let's consider the
    \k{subset} function from the R base library, which allows, among others, to
    select variables in a data frame. For example, given a data frame \k{df}
    with three columns \k{A,B,C}, \k{subset(df, sel=A)} returns a subset of
    \k{df} with just the \k{A} column. The same would happen with \k{subset(df,
      sel=1)}. However, thanks to \eval it can also be also called with
    \k{subset(df, sel=c(1,B,x))} returning a subset of the first column, column
    \k{B} and whatever index the variable \k{x} holds. It is implemented using:
    \lstinline|eval(substitute(sel), nl, parent.frame())| The first argument
    returns the expression passed to \k{sel}, the second is the data frame in
    which this expression will be evaluated, and the last indicates which
    environment should be used to look up bindings not found in \k{nl}.


 \emph{Assignments \k{(<-)}} appear in the cases where a developer wants
    ...
    \k{parse} or \k{substitute}, often in a loop. For example, the
    \k{PopED::get\_rse} uses it to assign a number of variables in the current
    scope: \lstinline|eval(parse(text=paste(default_args[[i]]), "<-", i))|.


 \emph{Function calls \k{(F(), F(X), F(F(X)))}} represent two very
    ...
    ad-hoc calls to libraries such as \k{ggplot2} when one needs to
    ...
    \k{A} and \k{B} columns in \k{df}. It might not be immediately
    clear\footnote{\cf
    ...
    judicious use of \eval. For example, the \k{data.table} package uses \eval
    to call the \k{options} function with named arguments taken from a vector
    of strings:
