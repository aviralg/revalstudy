...
\emph{Remove Boilerplate}

For example, the PopED::get_rse uses it to assign a number of variables in the current scope: eval(parse(text=paste(default_args[[i]]), "← ", i)). There are also more sophisticated cases. The plyr::allocate_column function uses eval for a deferred assignment to fill missing values in a column. Package overture repeatedly evaluates R expressions capturing any assignments that happen in it as samples of Markov chains.
