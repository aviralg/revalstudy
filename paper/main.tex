...
simplifications, the full list is in our artifact. It is worth mentioning that
these simplifications are heuristics, in R \c{1+1} is not necessary \c 2. The
addition operator can be redefined, but it typically isn't or at least not in a
way that would invalidate arithmetic.
...
\medskip\EE{$min(e)=$~\c{<-}} This form includes both assignment
...
\medskip\EE{$min(e)=\c{F()}$} This form captures simple function calls,
...
\medskip\framebox{$min(e)=\c{F(X)}$}~\EE{$min(e)=\c{F(F(X))}$} These
...
\medskip\EE{$min(e)=\c{FUN}$} This form captures expressions that
...
\medskip\EE{$min(e)=\c{BLOCK}$} This form captures multi-statement
...
\medskip\EE{$min(e)=\c{model.frame}$} The \c{model.frame} function
