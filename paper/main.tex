...
\end{tabular}
  ...
  \includegraphics[width=5.6cm, trim=5.5cm 0 5cm 0, clip]{calls_per_run_per_call_site}
...
\begin{tabular}{c}
  {\hspace{-3cm}\includegraphics[width=0.8\textwidth]{size_loaded_distribution}}
\end{tabular}
...
is the percentage of sites for which the \c{envir} argument is explicitly passed
to \eval and is not \c{parent.frame()}. The example column shows one sample
...
\begin{tabular}{|c|r|r|r|r|r|c|}\hline
  ...
  % macros are properly rounded and show the percentage sign now
\c X&\packageMinimizedcallsitesa &\packageMinimizedpropsitesa &\packageMinimizedpackagea &\packageMinimizedmedianoperationsaRnd &\packageMinimizedpercentenvirnonparenta & \c{y+1}\\\hline
\c{F(F(X))} & \packageMinimizedcallsitesb  & \packageMinimizedpropsitesb & \packageMinimizedpackageb  & \packageMinimizedmedianoperationsbRnd & \packageMinimizedpercentenvirnonparentb & \c{gbov( mean(x), a-1)}\\\hline
\c{V}&\packageMinimizedcallsitesc &\packageMinimizedpropsitesc &\packageMinimizedpackagec &\packageMinimizedmedianoperationscRnd &\packageMinimizedpercentenvirnonparentc& \c{c(42,21,0)}\\\hline
\c{F(X)}& \packageMinimizedcallsitesd & \packageMinimizedpropsitesd & \packageMinimizedpackaged & \packageMinimizedmedianoperationsdRnd & \packageMinimizedpercentenvirnonparentd & \c{seq\_len(iters)} \\\hline
\c{model.frame} & \packageMinimizedcallsitese & \packageMinimizedpropsitese & \packageMinimizedpackagee & \packageMinimizedmedianoperationseRnd & \packageMinimizedpercentenvirnonparente & \c{model.frame(formula = Z ~ U)} \\\hline
\c{F()}& \packageMinimizedcallsitesf & \packageMinimizedpropsitesf & \packageMinimizedpackagef & \packageMinimizedmedianoperationsfRnd & \packageMinimizedpercentenvirnonparentf & \c{rgamma(3, 2, n = 10L)} \\\hline
\c{\$}& \packageMinimizedcallsitesg & \packageMinimizedpropsitesg & \packageMinimizedpackageg & \packageMinimizedmedianoperationsgRnd & \packageMinimizedpercentenvirnonparentg & \c{DF\$B} \\\hline
\c{FUN} & \packageMinimizedcallsitesh & \packageMinimizedpropsitesh & \packageMinimizedpackageh & \packageMinimizedmedianoperationshRnd & \packageMinimizedpercentenvirnonparenth & \c{function(x, y) x + 3 * y} \\\hline
\c{<-} & \packageMinimizedcallsitesi  & \packageMinimizedpropsitesi & \packageMinimizedpackagei & \packageMinimizedmedianoperationsiRnd & \packageMinimizedpercentenvirnonparenti & \c{x[1, 2:3, 2:3] <- value}\\\hline
\c{\{\}} & \packageMinimizedcallsitesj & \packageMinimizedpropsitesj & \packageMinimizedpackagej & \packageMinimizedmedianoperationsjRnd & \packageMinimizedpercentenvirnonparentj & \c{\{ write.csv(iris, tf) ; file.size(tf) \}} \\\hline
\end{tabular}
\caption{Minimized expressions} \label{tab:minimizedexpressions}
\end{table}
% Other macros that can be used:
% - \packageMinimizedMedianOperations: median number of operations
% - \packageMinimizedPercentEnvir: percentages of sites with the default argument
% - \packageMinimizedPercentCallsEnvir: percentage of calls with the default envir argument
...
\medskip\noindent\EE{$min(e)=\c{V}$} Expression that represent values are
frequently passed to \eval -- they represent 17\% of the calls. The majority of
those, \packageValOneNodePercent, are inline constants (integers or doubles).
The rest trivially evaluate to a value, \eg~\c{1+1}.\footnote{True as long as
base functions such as \c{+} are not redefined. They typically aren't, but this
is a limitation of this categorization.} In our corpus,
...
replaced by a more interesting expression.
...
thus resulting in an arbitrary amount of computation.
...
occurs in 6\% of the sites. Arbitrary computations can occur if one of the
variables is bound to a promise.
...
sites. They represent the most obvious source of side-effects.
...
\medskip\noindent\EE{$min(e)=\c{F(X)}$} This form allows for a simple function
call whose arguments may include variable references. Assignments are excluded
from this form. It occurs in 14\% of the sites in the corpus.

\medskip\noindent\EE{$min(e)=\c{F(F(X))}$} This form extends the previous one
with nested function calls. It is the second most common argument to \eval,
occurring in 20\% of the sites.

\medskip\noindent\EE{$min(e)=\c{FUN}$} This form captures expressions that are
function definitions, \eg~\c{function(x)x+1}. \c{FUN} occurs in
\packageFunctionDefinitionSitesPercent of sites. One data point we have not
recorded is the side effects that are performed when the generated functions are
eventually run. In addition to \c{FUN} there
\packageGeneralizedFunctionDefinitionSitesPercent of sites create functions
nested in other expressions, this gives an idea of the use of higher-order
functions.

\medskip\noindent\EE{$min(e)=\c{BLOCK}$} This form captures blocks of code with
multiple statements. These are larger expressions, we do not inspect contents of
the blocks. They occur in 4\% of the sites.

\medskip\noindent\EE{$min(e)=\c{model.frame}$} This is a very specific form
intended to capture the construction of calls to the \c{model.frame} function.
This function creates statistical models. This subsumes \c{F(F(X))}, \c{FUN} and
assignments. This particular form occurs in 7\% of the sites.
...
environments can be created using \c{new.env}, or \c{list2env}. They can be provided a parent
environment which becomes the enclosing scope of the new environment.
% as.environment also can be used sometimes for that
% list2env creates an environment only if its argument envir is not provided
...
\c{NULL}, a \c{list} or a data frame, or an integer: \eval copies the fields of the
...
% This table is old and incorrect
...
% 85% (\packageEnclosWithListPercent) actually have a list \c{(envir)}. The rest of
% of them have environment \c{envir}. In that case, \eval ignores \c{enclos} anyway
...
% Explain that some generate expression objects and other language objects?
...
\begin{itemize}
    ...
    \item \c{quote}, \c{enquote} returns its argument unevaluated, as an R expression. % Aldd \c{call} ?
...
\end{itemize}
We show the distribution of the different provenances in Table~\ref{tab:provenance}. Note that one site could be passed arguments from different provenances depending on the invokation. For some arguments, we could not determine their provenance or it was a value or a symbol, for which the provenance is less significant. % Not clear... Or maybe add symbols in that table?
% One can create a symbol with as.name, as.symbol etc. That would be the provenance (but a symbol can also come from quote for instance...)
...
% explain also how we get the numbers of the other categories?
...
\mypara{Variable lookup.} % predominant usage of eval in base

\mypara{Symbolic differentiation.} % D and deriv
