...
The environment in which evaluation happens determines what will be visible to
the computation started by \eval, and the potential reach of side-effects. The
second argument to \eval, \c{envir}, defines the evaluation environment of the
expression. That argument can be omitted, in which case the default value is
\c{parent.frame()} which returns the caller of the \eval function.

Other possible values for the environment include, lists, dataframes, pairlists,
integers and \c{Null}. Lists and dataframe are coerced to new environments,
integers values are passed to the \c{sys.call} function which returns the
...
evaluation of the rest of the function. This can be useful for static analyzers.
