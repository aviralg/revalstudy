...
\begin{table}[h]
	\begin{tabular}{|c|c|c|c|c|c|}
		...
		\hline
	\end{tabular}
	\caption{Minimized expressions: ten most frequent in terms of sites. \#sites is the number of sites; \%sites the percentage of sites across the total number of sites (\packageNbCallSites sites) with at least this minimized expression; \#packages is the number of packages with at least one of the minimized expression and measures how widespread a minimized expression is. \#operations is the average number of steps performed by the R interpreter when given that minimized expression, and \%envir is the percentage of call sites for which the \c{envir} argument is explicitly passed.} \label{tab:minimizedexpressions}
\end{table}


Note that a site can exhibit several minimized expressions. However, \packageNbOneMinimizedPercent of the sites have only one minimized expression. 
...
\paragraph{Composition of functions. } Compositions of functions, without variables, is the second most frequent minimized expression. 
...
\begin{itemize}
	\item \eval is passed an actual value, a double, an integer, an environment, a closure and so on
	\item \eval is passed a language expression that trivially evaluates to a value, for instance, \c{quote(1)}, \quote{c("a", "b", "c")}, and does not include variables, so does not depend on a specific environment.
\end{itemize}

For the first category, \eval is not necessary. In \packageNbCallSitesUniqueActualValue sites, \ie \packageCallSitesUniqueActualValuePercent\% sites, it is only passed such actual values. The median number of runs for those sites is \packageMedianRunSitesUniqueActualValue, suggesting it happens because there are rarely triggered in our dataset and all paths are not taken.

The percentage of non default environment passed to \eval is \packageMinimizedpercentenvirc. Some sites are passed both a value or a symbol, which explains that a specific environment is precised. 

%TODO: find out the percentage of non-default envir for sites which have both a value and a variable
%TODO: find the same percentage for sites which have at least one variable passed to them?

When a function is evaluated on some variable (\c{F(X)}), the percentage of non default environment is high, \packageMinimizedpercentenvird\%.

\c{FUN} represents a function definition. It entails very few steps of the interpreter, as the body of the new function is just stored for later execution.
