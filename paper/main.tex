...
offset on the call stack. Thus 0 is the direct caller and 1 is its parent, and
so on. The data suggests that in 81\% of cases, \eval access its caller's
environment -- this means, the variables of the function where \eval textually
...
distance implies that, in general, it is modular reasoning is impossible, to
...
kind of environments, but in \packageNbOneCategoryEnvirSitePercent of the case a
...
Synthetic environments need a parent. For this, \eval uses its third argument
(\c{enclos}). When a synthetic environment is created by \eval, it has
\c{enclos} as a parent. The parent is used to lookup variables not found in its
...
evaluate an expression in either a restricted environment or use a data
