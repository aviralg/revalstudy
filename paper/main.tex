...
library code and the \Corpus packages in the CRAN corpus. Data was
collected from \Nbruns runs of code extracted from the target packages
and their reverse dependencies. In total, \Allcalls invocations of
\eval and its variants were recorded. Runs that did not call \eval
were discarded.\footnote{The fact that a package contains call sites
to \eval does not mean every run will execute those call sites.} Out
of the \Staticatleastonecallsite packages that use \eval,
\Triggeredpkgs packages we recorded calling the function --- the
difference can be chalk down to incomplete code coverage and
occasional failures in our pipeline.
...
Figure~\ref{tab:variantseval} shows, per variant of \eval, the number
of call sites in the corpus (\emph{static}), the number of call sites
that were observed during tracing (\emph{exercised}), and the total
number of dynamic calls (\emph{invocations}). The data clearly shows
that the variants of \eval are rarely used, in particular \c{evalq}
and \c{local}.

\begin{figure}[h]\begin{tabular}{|r|r|r|r|r|}\hline
...
Invocations&\EvalsRnd&\EvalqsRnd&\EparentsRnd&\LocalsRnd\\\hline
...
Figure~\ref{tab:callsitesnormalized} shows the number of invocation to
\eval and its variants per call sites per run exercising that call
site. If that number is high, it can correspond to a function with an
\eval call site that is used widely. If that number is very high, more
than 1000, we expect it to correspond to an \eval call site in a loop.
Most of the invocations are less than 50 per run per call site. When
zooming in, on Figure~\ref{fig:callsitesnormalized}, most call sites
have actually only one invocation per run.


\begin{figure}[h] \centering
    \includegraphics[width=.5\textwidth]{calls_per_run_per_call_site}
    \caption{Invocations of \eval per call sites normalized by the
      number of runs, for less than 50 normalized invocations. Binsize
      is 1} \label{fig:callsitesnormalized}
\end{figure}
...
bytes. The average size is \Avgszeval bytes.
