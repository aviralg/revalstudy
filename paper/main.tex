...
\noindent The expression passed to \eval can be thought of as an abstract syntax
...
\noindent The call to \c{substitute(x)} extracts the unevaluated expression from
...
\noindent The pipeline runs in parallel~\cite{GNUparallel} orchestrated by a
...
%% UNFINED TODO \packageUsefulValueEvalPercent
...
of numerous examples suggests that errors are rare.
...
% TODO connect the minimization group and side-effects
...
\emph{Simplify Interfaces}. A common use of \eval is to simplify interfaces by
...
\emph{Code Generation.} The \c{Rcpp} package which bridges R with C++ is a
...
\emph{Extracting ... arguments.} A surprising use of \eval is the extraction of unevaluated text of arguments
passed to a \code{...} parameter. Consider the definition of the \code{NVL}
function from the \code{statnet.common} package. This function returns the first
argument that is non \code{NULL}.
...
\emph{Conditional Evaluation.} Developers frequently use \eval to explicitly
...
\emph{Core function variants.} Many packages use \eval to implement variations of the
...
\emph{Domain-Specific Languages.} The combination of laziness, metaprogramming,
...
\emph{Easier Maintenance.} The combination of \c{match.call} and \eval is
frequently used to make it easier to maintain functions with similar interfaces.
This pattern is very commonly employed by modeling functions. Consider the
example below of the \code{survfit.formula} function from package
\code{survival}.
...
This function uses \code{match.call} to extract the expression with which it is
called. Then, it extracts the indices of relevant arguments using the
\code{match} function. Next, it subsets the \code{Call} variable to extract
elements at those indices along with the first element which is the function
name. The function name is then replaced with \code{stats::model.frame}.
Finally, this expression is evaluated. By using \code{...}, the function accepts
arbitrary arguments. It filters the arguments of interest using \code{match}
function. \todo{explain more}


\emph{Remove Boilerplate.} For example, the \code{PopED::get\_rse} uses it to
...
\emph{Code Obfuscation.} Some packages use \eval to bypass the restrictions
...
this use is the \code{scalaEnsure} function from the \code{aibd} package. It
mutates the variable \code{s} in the package environment. However, the package
environment in R is locked by default, which prevents bindings from being
mutated. Environment can be unlocked using \code{unlockBinding}, but its use is
flagged by the well-formedness checks. To work around this limitation, the
author passes the expression to unlock the package environment as a string to
\code{parse} which is then evaluated by \code{eval}.
