...
the text can refer to some variable \c x that will be set up by the called
...
construct function objects that are parameterized with string \c{what}.
...
\emph{Variable lookup and values, and indexing \c{(X, V, \$)}}
 ...
 \emph{Assignments \c{(<-)}} appear in the cases where a developer wants
 ...
 \emph{Function calls \c{(F(), F(X), F(F(X)))}} represent two very
...
such as \c X, \c V, \$, \c{<-}, and \c{FUN}. Variable lookup in a remote
...
\c{do.call} function to build function calls. Unnecessary uses of \eval
...
function body uses \c{get} to perform the lookup and then calls directly.
...
Another example, from \c{coin::copyslots}, constructs an expression that access
...
that are then fed to \eval. Semantically equivalent code would use \c{get}
to lookup \c{fun}, and then directly execute the three expressions.
...
\c{rlang::eval\_tidy}~\cite{tidyverse}. This departs from \eval in two ways.
First, it supports evaluation of quosures, custom objects used by for
metaprogramming that bundle expressions with an environment. \c{eval\_tidy}
...
in the data mask. Expressions such as \c{return()} do not work since the data
